    ...
    public class BlLwmcApiStakeholderHander
    {
        ...
        public static LwmcStakeholderComplaintResponse GetStakeHolderComplaintsServerSideByUserNameDynamic(string fcmKey, string userName, string statuses, int startingRowIndex, string from, string to, Config.Language language, Config.PlatformID platformId) //string commaSeperatedCampaigns, string commaSeperatedCategories, string commaSeperatedStatuses, string commaSeperatedTransferedStatus, int complaintsType, Config.StakeholderComplaintListingType listingType, string spType, int userId = -1)
        {
            try
            {
                ...
                {
                    paramsSchoolEducation = SetStakeholderListingParams(dbUser, startingRowIndex, from, to, dbUser.Campaigns, dbUser.Categories, statuses, "1,0", Config.ComplaintType.Complaint, Config.StakeholderComplaintListingType.UptilMyHierarchy, "MobileListingWithDateFilters");
                }
                else
                {
                    paramsSchoolEducation = SetStakeholderListingParams(dbUser, startingRowIndex, from, to, dbUser.Campaigns, dbUser.Categories, statuses, "1,0", Config.ComplaintType.Complaint, Config.StakeholderComplaintListingType.AssignedToMe, "MobileListingWithDateFilters");
                }
                ...
                if (dbUser != null && dbUser.Role_Id == Config.Roles.Stakeholder)
                {
                    ...
                    foreach (LwmcStakeholderComplaint complaint in listStakeholderComplaints)
                    {
                        ...
                        complaint.Created_Date = Utility.GetDateTimeStr(complaint.Created_Date, "dd/MM/yyyy");
            ...
            List<Picture> listPictures, Int64 apiRequestId)
        {
            ...
            return StatusHandler.ChangeStatus(username, complaintId, statusId, statusComments, listPictures, apiRequestId,
                canSendMessage);
        }
