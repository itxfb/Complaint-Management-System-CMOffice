...
namespace PITB.CMS_Common.Models
{
    ...
    public partial class DbUsers
    {
        ...
        public static Config.CommandStatus UpdatePublicUserProfile(int userId, string name, string phone)
        {
            try
            {
                DBContextHelperLinq db = new DBContextHelperLinq();
        ...
        public static Dictionary<string, string> GetCrlActionsDict(DbUsers dbUser)
        {
            //string.Format("Role")
            Dictionary<string, string> permDictToRet = null;
            List<DbPermissionsAssignment> listPermAssign = DbPermissionsAssignment.GetListOfPermissionsByTypeTypeIdAndPermissionId((int)Config.PermissionsType.User, dbUser.User_Id, (int)Config.Permissions.AllowedCtrlActionList);
            string tagToMatch = GetTagToMatchForUserPages(dbUser);
            // if Usercontroller action present in user permission
            if (listPermAssign.Count > 0)
            {
                if (listPermAssign.FirstOrDefault() != null)
                {
                    permDictToRet = JsonConvert.DeserializeObject<Dictionary<string, string>>(listPermAssign.FirstOrDefault().Permission_Value);
            ...
            else
            {
                ...
                listPermAssign = DbPermissionsAssignment.GetListOfPermissionsByTypeTypeIdListAndPermissionId((int)Config.PermissionsType.Campaign, Utility.GetIntList(dbUser.Campaigns), (int)Config.CampaignPermissions.AllowedCtrlActionList);
                if (listPermAssign.FirstOrDefault() != null)
                {
                    permDictToRet = JsonConvert.DeserializeObject<Dictionary<string, string>>(listPermAssign.FirstOrDefault().Permission_Value);
                    return permDictToRet;
                ...
                else
                {
                    ...
                    //listPermAssign = DbPermissionsAssignment.GetListOfPermissionsByTypeTypeIdAndPermissionId((int)Config.PermissionsType.Campaign, -1, (int)Config.CampaignPermissions.AllowedCtrlActionList);
                    List<DbConfiguration_Assignment> listDbConfigAssign = DbConfiguration_Assignment.GetByTagId(string.Format("{0}__{1}", Config.CONFIG_TAG_ALLOWED_CTRL_ACTION_LIST, "RoleId::" + ((int)dbUser.Role_Id)));
        ...
        private static string GetTagToMatchForUserPages(DbUsers dbUsers)
        {
            string tag = "";
            if (dbUsers.Role_Id == Config.Roles.Agent || dbUsers.Role_Id == Config.Roles.AgentSuperVisor)
            {
                tag = string.Format("RoleId::{0}", (int)dbUsers.Role_Id);
            }
            else if (dbUsers.Role_Id == Config.Roles.Stakeholder && dbUsers.Campaigns.Contains(((int)Config.Campaign.SchoolEducationEnhanced).ToString()))
            {
                tag = string.Format("RoleId::{0}{1}CampaignId::{2}{3}UserId::{4}", (int)dbUsers.Role_Id, "__", dbUsers.Campaigns, "__", dbUsers.User_Id);
            }
            else //if (dbUsers.Role_Id==Config.Roles.Stakeholder)
            {
                tag = string.Format("RoleId::{0}{1}CampaignId::{2}", (int)dbUsers.Role_Id, "__", dbUsers.Campaigns);
            }
            return tag;
        }
            ...
            List<DbProvince> dbProvince, List<DbDivision> dbDivision, List<DbDistrict> listDistrict, List<DbTehsil> listTehsil, List<DbUnionCouncils> listUc)
        {
            ...
            if (!this.Username.Contains(syncUsers.crmu_cnic.TrimNullable())
                ...
                || !syncUsers.crmu_email.IsEqualAfterTrim(this.Email.TrimNullable())
                ...
                )
            {
                ...
                this.Email = syncUsers.crmu_email.TrimNullable();
            ...
            int campaignId, List<DbComplaintType> listDbComplaintType, List<DbUsers> listDbUsers)
        {
            ...
            dbUser.Email = syncUsers.crmu_email;
    ...
    public partial class DbUsers
    {
        ...
        public static string GetUserHashString(DbUsers dbUser)
        {
            dynamic d = new ExpandoObject();
            d.userId = dbUser.User_Id;
            //d.name = dbUser.Name;
            //d.username = dbUser.Username;
            //d.email = dbUser.Email;
            //d.cnic = dbUser.Cnic;
            //d.contactNo = dbUser.Phone;
            //d.designation = dbUser.Designation;
            //d.designationAbbr = dbUser.Designation_abbr;
            return Utility.GetEncryptedString(Utility.SerializeDynamic(d));
        }

        public static int GetUserIdFromHashString(string hashStr)
        {
            //dynamic d = new ExpandoObject();
            //d.userId = dbUser.User_Id;
            //d.name = dbUser.Name;
            ...
            //d.designation = dbUser.Designation;
            //d.designationAbbr = dbUser.Designation_abbr;
            return Convert.ToInt32(Utility.DeserializeToDynamic(Utility.GetDecryptedString(hashStr)).userId);
        }

        public static dynamic GetUserHashModel(string hashStr)
        {
            return Utility.DeserializeToDynamic(Utility.GetDecryptedString(hashStr));
        }
