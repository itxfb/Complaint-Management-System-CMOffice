...
namespace PITB.CMS_Common.Handler.Sync.SchoolEducation
{
    public class SyncSchoolDataHandler
    {
        ...
        public static void SynRegionAndShoolsDataMain(DateTime from, DateTime to)
        {
            try
            {
                ...
                for (int i=100; i < listDataRequired.Count; i++)
                {
                    try
                    {
                        ...
                        else if (listDataRequired[i] == Config.SyncApiRequiredData.school)
                        {
                            ...
                            List<DbComplaint> listDbComplaint = DbComplaint.GetBy((int)Config.Campaign.SchoolEducationEnhanced, new List<int> { (int)Config.ComplaintStatus.PendingFresh, (int)Config.ComplaintStatus.PendingReopened, (int)Config.ComplaintStatus.UnsatisfactoryClosed });
        ...
        public static bool SynTehsils(/*DBContextHelperLinq db,*/ List<DbCrmIdsMappingToOtherSystem> listCrmIdsMapping, List<DbTehsil> listDbTehsils, ApiResponseSyncSEModel.ResponseSEDataTehsil responseTehsil)
        {
            ...
            {
                DbWindowServiceError.SaveErrorLog(1, Environment.StackTrace, ex, Config.ServiceType.SyncSchoolDataMain);
                throw;
            }
            finally
            {
            ...
            List<DbProvince> listProvinces, List<DbDivision> listDivisions, List<DbDistrict> listDistricts, List<DbTehsil> listTehsils, List<DbUnionCouncils> listUc)
        {
            ...
            try
            {
                ...
                //respnseUsers.data = respnseUsers.data.Where(n => n.crmu_id == "52309").ToList();
                //-------- end test code -----
