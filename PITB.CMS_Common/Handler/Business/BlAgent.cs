...
namespace PITB.CMS_Common.Handler.Business
{
    public class BlAgent
    {
        public static DataTable GetComplaintListings(DataTableParamsModel dtModel, string fromDate, string toDate, string campaign, int complaintType, string spType, int listingType)
        {
            ...
            if (cookie.Role == Config.Roles.AgentSuperVisor)
            {
                tagPrefix = "Supervisor";
            }
            else if (cookie.Role == Config.Roles.Agent)
            {
                tagPrefix = "Agent";
            }
            //if (cookie.Role == Config.Roles.Agent)
            //{
            ...
            if (listingType == (int)Config.AgentComplaintListingType.AddedByMe)
            {
                ...
                {
                    queryStr = QueryHelper.GetFinalQuery(string.Format("{0}_ComplaintsListing_Mine{1}", tagPrefix, exportStr), Config.ConfigType.Query,
                        dictParam);
                }
                if (complaintType == (int)Config.ComplaintType.Suggestion)
                {
                    queryStr = QueryHelper.GetFinalQuery(string.Format("{0}_ComplaintsListing_Suggestion{1}", tagPrefix, exportStr), Config.ConfigType.Query,
                            dictParam);
                }
                else if (complaintType == (int)Config.ComplaintType.Inquiry)
                {
                    queryStr = QueryHelper.GetFinalQuery(string.Format("{0}_ComplaintsListing_Inquiry{1}", tagPrefix, exportStr), Config.ConfigType.Query,
                            dictParam);
                }
            }
            else if (listingType == (int)Config.AgentComplaintListingType.All)
            {
                queryStr = QueryHelper.GetFinalQuery(string.Format("{0}_ComplaintsListing_All{1}", tagPrefix, exportStr), Config.ConfigType.Query,
                    dictParam);
            }
            //}
