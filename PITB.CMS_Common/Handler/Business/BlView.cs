...
using PITB.CMS_Common.Controllers;
using System.Web.Mvc;
using Controller = PITB.CMS_Common.Controllers.View.Controller;
...
namespace PITB.CMS_Common.Handler.Business
{
    public class BlView
    {
        ...
        public void SetStartEndDate(Controller ctrlRef)
        {
            CMSCookie cmsCookie = AuthenticationHandler.GetCookie();
            if (cmsCookie.navigationalData != null)
            {
                if (cmsCookie.navigationalData.StartDate != null)
                {
                    ctrlRef.ViewBag.StartDate = cmsCookie.navigationalData.StartDate;
                    ctrlRef.ViewBag.EndDate = cmsCookie.navigationalData.EndDate;
                }
            }
            else
            {
                ctrlRef.ViewBag.StartDate = DateTime.Now.AddMonths(-3).ToString("yyyy-MM-dd");
                ctrlRef.ViewBag.EndDate = DateTime.Now.ToString("yyyy-MM-dd");
            }
        }
        ...
        public ActionResult GetComplaintDetail(int complaintId, string viewType, Controller ctrlRef)
        {
            ...
            else if (viewType == "PublicUserComplaintDetail")
            {
                VmComplaintDetailPublicUser vmComplaintDetail = new VmComplaintDetailPublicUser(BlComplaints.GetComplaintDetail(dbComplaint));
                vmComplaintDetail.vmFileModel = FileHandler.GetVmFileModel(complaintId, (int)Config.AttachmentReferenceType.Add, complaintId);
                //ctrlRef.ViewBag.listSatisfaction = new List<object>() { new { key = 1, value = "Satisfied" }, new { key = 2, value = "dissatisfied" } };
                //ctrlRef.ViewBag = 1;
                //vmComplaintDetail.ViewTag = viewType;
                if (vmComplaintDetail.VmStatusChange.currentStatusId==(int)Config.ComplaintStatus.ResolvedVerified)
                {
                    ctrlRef.ViewBag.listStatuses = DbStatus.GetByStatusIds(new List<int>() { 34, 35 });
                }
                
                return ctrlRef.ReturnView("~/Views/PublicUserComplaints/_ComplaintDetail.cshtml", null, vmComplaintDetail);
            }

            return null;
        }

        public void UpdateUserLogin(int currentUserId, CMSCookie prevCmsCookie, CMSCookie cookieToNavigate)
        {
            //CMSCookie prevCmsCookie = AuthenticationHandler.GetCookie();
            //var previousUserId = prevUserId; //prevCmsCookie.UserId;
            //int userId = cmsCookie.navigationalData.perviousUserId; //cmsCookie.navigationalData.perviousUserId
            //DbUsers dbUser = DbUsers.GetActiveUser((int)prevCmsCookie.PreviousLoginId);

            //if (cmsCookie.PreviousLoginId != null)
            //    cmsCookie.PreviousLoginId = (int)cmsCookie.PreviousLoginId;

            //cmsCookie = CMSCookie.DbUserToCookie(dbUser);
            //cookieToNavigate.navigationalData = prevCmsCookie.navigationalData;
            cookieToNavigate.PreviousLoginId = currentUserId;
            //cookieToNavigate.UrlConfig = null;
            cookieToNavigate.navigationalData = prevCmsCookie.navigationalData;
            new AuthenticationHandler().LogOut();
            new AuthenticationHandler().SaveCookie(cookieToNavigate);
        }

        public static string GetAlteredCampaignName(CMSCookie cmsCookie, string campaignName)
        {
            string alteredCampaignName = "";
            if (cmsCookie.navigationalData != null && cmsCookie.navigationalData.campaignName != null/* && dbCamp.Campaign_Name.ToLower() == "dc-office"*/)
                alteredCampaignName = cmsCookie.navigationalData.campaignName;
            else
            {
                alteredCampaignName = campaignName;
            }
            return alteredCampaignName;
        }
        ...
        private string GetMasterPage(CMSCookie cookie)
        {
            ...
            if (cookie.Role == Config.Roles.Stakeholder)
            {
                ...
                else if (cookie.Campaigns == Convert.ToInt32(Config.Campaign.DcChiniot).ToString())
                {
                    masterPage = "~/Views/Shared/DcChiniot/_DcChiniotStakeholderLayout.cshtml";
                }
                else
                {
            ...
            if (cookie.Role == Config.Roles.ExecutiveCampaigns)
            {
                masterPage = "~/Views/Shared/ExecutiveCampaigns/_ExecutiveCampaignsLayout.cshtml";
            }
            //------------------
        ...
        public ActionResult GetListingView(int ComplaintType, string viewType, Controller ctrlRef)
        {
            ...
            if (viewType == "ComplaintsMy")
            {
                ...
                // if is school education new campaign
                {
                        ...
                        Utility.IsElementPresentInString(cookie.Campaigns, (int)Config.Campaign.DcoOffice))
                    {
                        listStatuses =
                            BlCommon.GetStatusListByCampaignIds(
                                ...
                                Config.Permissions.StatusesForComplaintListing);
                        ctrlRef.ViewBag.StatusList = listStatuses;
                        ctrlRef.ViewBag.CampaignName = BlView.GetAlteredCampaignName(cookie, ctrlRef.ViewBag.CampaignName);
                        return
                            ctrlRef.ReturnView(
                    ...
                    if (Utility.IsElementPresentInString(cookie.Campaigns, (int)Config.Campaign.DcChiniot))
                    {
                        listStatuses =
                            BlCommon.GetStatusListByCampaignIds(
                                listCampaings.Select(n => Convert.ToInt32(n.Value)).ToList(),
                        ...
                        ctrlRef.ViewBag.StatusList = listStatuses;
                        ctrlRef.ViewBag.CampaignName = BlView.GetAlteredCampaignName(cookie, ctrlRef.ViewBag.CampaignName);
                        return ctrlRef.ReturnView("~/Views/Stakeholder/DcChiniot/DcChiniotComplaintListingsServerSide.cshtml");
                    }
                ...
                if (Utility.IsAllMultipleElementsPresentInString(cookie.Campaigns, "69,68,72,73,74"))
                {
                    ...
                    return
                        ctrlRef.ReturnView(
                            "~/Views/Stakeholder/Health/ZimmerdarShehriComplaintListingsServerSide.cshtml");
                }
                listStatuses =
                    BlCommon.GetStatusListByCampaignIds(
                        listCampaings.Select(n => Convert.ToInt32(n.Value)).ToList(),
            ...
            else if (viewType == "ComplaintsLowerHierarchy")
            {
                ...
                // if is school education new campaign
                {
                    if (ctrlRef.ViewBag.StartDate == DateTime.Now.AddMonths(-3).ToString("yyyy-MM-dd"))
                    {
                        ctrlRef.ViewBag.StartDate = DateTime.Parse("01/08/2018").ToString("yyyy-MM-dd");
                        ctrlRef.ViewBag.EndDate = DateTime.Now.ToString("yyyy-MM-dd");
                    }
                ...
                if (Utility.IsElementPresentInString(cookie.Campaigns, (int)Config.Campaign.DcChiniot))
                {
                    listStatuses = Utility.GetAlteredStatus(listStatuses, Config.UnsatisfactoryClosedStatus, Config.UnsatisfactoryClosedStatus);
                    ctrlRef.ViewBag.CampaignName = BlView.GetAlteredCampaignName(cookie, ctrlRef.ViewBag.CampaignName);
                    return ctrlRef.ReturnView("~/Views/Stakeholder/DcChiniot/DcChiniotComplaintListingsLowerHierarchyServerSide.cshtml");
                }
            ...
            else if (viewType == "ComplaintsSuggestion")
            {
                ...
                else if (Utility.IsElementPresentInString(cookie.Campaigns, (int)Config.Campaign.DcChiniot))
                // if is school education new campaign
                {
                    return ctrlRef.ReturnView("~/Views/Stakeholder/DcChiniot/DcChiniotSuggestionListingsServerSide.cshtml");
                }
                else
                {
            ...
            else if (viewType == "ComplaintsInquiry")
            {
                ...
                // if is school education new campaign
                {
                    return ctrlRef.ReturnView("~/Views/Stakeholder/DcChiniot/DcChiniotInquiryListingsServerSide.cshtml");
                }
                else
                {
