    ...
    public class BlCommon
    {
        ...
        public static int Export([FromBody] JToken jsonBody)
        {
            try
            {
                ...
                List<int> listCampaigns = new List<int>();

                foreach (var x in commaSeperatedCampaigns.Split(','))
                {
                    listCampaigns.Add(Convert.ToInt32(x));
                }
                ...
                DataTable data = null;
                int val = 2;
                if (listCampaigns.Contains((int)Config.Campaign.ZimmedarShehri))
                {
                ...
                else if (listCampaigns.Contains((int)Config.Campaign.FixItLwmc))
                {
                    DataTableParamsModel dtModel = DataTableHandler.ConvertaoDataToModel(dict["aoData"]);
        ...
        */
        ...
        public static List<SelectListItem> GetStatusListByCampaignIds(List<int> listCampaignIds, Config.Permissions userPermissions)
        {
            ...
            statusList.Add(new DbStatus() { Complaint_Status_Id = -1, Status = "Total" });
        ...
        public static List<DbStatus> GetStatusListByCampaignIds_Permission(List<int> listCampaignIds, int userId, CMSCookie cookie, Config.Permissions permission)
        {
            //CMSCookie cookie = AuthenticationHandler.GetCookie();
        ...
        public static List<DbStatus> GetStatusListByCampaignIds_Permissions_Status(List<int> listCampaignIds, int userId, List<DbPermissionsAssignment> listPermissions, List<DbStatus> listDbStatuses, Config.Permissions permission)
        {
            //CMSCookie cookie = AuthenticationHandler.GetCookie();
            ...
            {
                listStatuses = Utility.GetIntList(dbPermission.Permission_Value);
                statusList = listDbStatuses.Where(n => listStatuses.Contains(n.Complaint_Status_Id)).ToList();
            }
            else
            {
                List<DbStatus> listDbStatus = listDbStatuses.Select(n => n).ToList();
                statusList = listDbStatus.Where(n => Utility.GetIntList(n.Campaigns).ToList().Any(p => listCampaignIds.Contains(p))).ToList();
            }
