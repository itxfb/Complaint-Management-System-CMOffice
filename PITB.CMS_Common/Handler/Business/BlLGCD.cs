...
namespace PITB.CMS_Common.Handler.Business
{
    public class BlLGCD
    {
        public static Config.CommandMessage AddComplaint(CustomForm.Post form)
        {
            ...
            if (currentTab == VmAddComplaint.TabComplaint || currentTab == VmAddComplaint.TabComplaintCombined)
            {
                modelName = "ComplaintVm";
                complaintType = (int)Config.ComplaintType.Complaint;
            }
            else if (currentTab == VmAddComplaint.TabSuggestion || currentTab == VmAddComplaint.TabSuggestionCombined)
            {
                modelName = "SuggestionVm";
                complaintType = (int)Config.ComplaintType.Suggestion;
            }
            else if (currentTab == VmAddComplaint.TabInquiry || currentTab == VmAddComplaint.TabInquiryCombined)
            {
                modelName = "InquiryVm";
            ...
            if (currentTab == VmAddComplaint.TabComplaint || currentTab == VmAddComplaint.TabComplaintCombined ) // when there is complaint populate assignment matrix
            {
            ...
            complaintSrc = (cmsCookie.Role == Config.Roles.PublicUser) ? (int)Config.ComplaintSource.Public :(int)Config.ComplaintSource.Agent;
            ...
            paramDict.Add("@Complaint_Remarks", HttpUtility.HtmlDecode(complaintRemarks).ToDbObj());
            paramDict.Add("@Agent_Comments", HttpUtility.HtmlDecode(complaintAgentComments).ToDbObj());
            ...
            paramDict.Add("@Person_Name", HttpUtility.HtmlDecode(personName).ToDbObj());
            paramDict.Add("@Person_Father_Name", HttpUtility.HtmlDecode(personFatherName).ToDbObj());
            ...
            paramDict.Add("@Person_Address", HttpUtility.HtmlDecode(personAddress).ToDbObj());
            paramDict.Add("@Email", HttpUtility.HtmlDecode(personEmail).ToDbObj());
            ...
            {
                Debug.WriteLine("key = " + keyVal.Key + " val = " + keyVal.Value);
            }
            ...
            if (cm.Status == Config.CommandStatus.Success && (currentTab == VmAddComplaint.TabComplaint || currentTab == VmAddComplaint.TabComplaintCombined)) // send message on complaint launch
            {
