...
using PITB.CMS_Common.Modules.Query.Template;
...
namespace PITB.CMS_Common.Handler.Business
{
    public class BlSchool
    {
        ...
        public static Config.CommandMessage AddComplaint(VmAddComplaint vm, bool isProfileEditing, bool isComplaintEditing)
        {
            ...
            List<AssignmentModel> assignmentModelList = null;
            if (vm.currentComplaintTypeTab == VmAddComplaint.TabComplaintCombined || vm.currentComplaintTypeTab == VmAddComplaint.TabComplaintCombined)
            {
        ...
        public static VmAddComplaint GetVmAddComplaintMerged(VmAddComplaint viewModel, int profileId = 0, int campaignId = 0)
        {
            ...
            dynamic dTagId = new ExpandoObject();
            dTagId.campaignId = campaignId;
            dTagId.moduleId = "department_filter";
            dTagId.filterKeyName = "schoolCategory";
            dTagId.filterKeyValue = 1;

            string tagId = Utility.GetSerializedJsonWithoutSpaces(dTagId);
            string tagVal = DbTagLookup.GetTag(tagId);
            //dTagId.
            viewModel.ComplaintVm.ListOfDepartment = DbDepartment.GetByCampaignAndGroupId(campaignId, groupId, tagVal);
            ...
            viewModel.SuggestionVm.ListOfDepartment = DbDepartment.GetByCampaignAndGroupId(campaignId, groupId, tagVal);
            ...
            viewModel.InquiryVm.ListOfDepartment = DbDepartment.GetByCampaignAndGroupId(campaignId, groupId, tagVal);
        ...
        public static string GetComplaintsAgainstUser_Query(DbUsers dbUser, List<DbPermissionsAssignment> listDbPermissionsAssignment, List<DbStatus> listStatuses, Config.StakeholderComplaintListingType listingType, Config.ComplaintType complaintType, Config.SelectionType selectionType)
        {
            string commaSeperatedCampaigns = dbUser.Campaigns;
            string commaSeperatedCategories = dbUser.Categories;
            ...
            List<DbPermissionsAssignment> userPermissions = listDbPermissionsAssignment.Where(m => m.Type == (int)Config.PermissionsType.User && m.Type_Id == dbUser.User_Id && m.Permission_Id == (int)StatusPermission).ToList();

            List<DbStatus> listDbStatuses = BlCommon.GetStatusListByCampaignIds_Permissions_Status(Utility.GetIntList(dbUser.Campaigns), dbUser.User_Id,
                        userPermissions, listStatuses, StatusPermission);

            string commaSeperatedStatuses = Utility.GetCommaSepStrFromList(listDbStatuses.Select(n => n.Complaint_Status_Id).ToList());
            string commaSeperatedTransferedStatus = "0,1";
            // Config.ComplaintType complaintType = 
            ListingParamsModelBase paramsSchoolEducation = SetParamsSchoolEducation_2(dbUser, null, null, commaSeperatedCampaigns, commaSeperatedCategories, commaSeperatedStatuses, commaSeperatedTransferedStatus, null, complaintType, listingType, "UserComplaintsList", listDbPermissionsAssignment);
            if (selectionType == Config.SelectionType.All)
            {
                paramsSchoolEducation.SelectionFields = "complaints.*";
            }
            ...
            {
                paramsSchoolEducation.SelectionFields = "complaints.Id,complaints.Agent_Id";
            }
            //paramsSchoolEducation.SelectionFields = 
            return StakeholderListingLogic.GetListingQuery(paramsSchoolEducation);
        }
        ...
        public static DataTable GetStakeHolderServerSideListDenormalized(string from, string to, DataTableParamsModel dtModel, string commaSeperatedCampaigns, string commaSeperatedCategories, string commaSeperatedStatuses, string commaSeperatedTransferedStatus, int complaintsType, Config.StakeholderComplaintListingType listingType, string spType, int userId = -1)
        {
            ...
            if (dtModel != null)
            {
                ...
                //string QueryTag = string.Format("RoleId::{0}{1}CampaignId::{2}{3}ModuleName::{4}", (int)dbUser.Role_Id, "__", dbUser.Campaign_Id, "__", spType);
                //string tagToMatch = QueryTag + string.Format("__UserId::{0}", dbUser.User_Id);
                //List<DbConfiguration_Assignment> listDbConfigAssign = DbConfiguration_Assignment.GetByTagId(tagToMatch);
                //if (listDbConfigAssign.FirstOrDefault() != null)
                //{
                //    tagToMatch += string.Format("__UserId::{0}", dbUser.User_Id);
                //    string matchedKey = TagHandler.GetMatchedTag(tagToMatch, listDbConfigAssign.Select(n => n.Key).ToList());
            ...
            Dictionary<string, object> queryParms = StakeholderListingLogic.GetQueryParams(paramsSchoolEducation);

            string QueryTag = string.Format("RoleId::{0}{1}CampaignId::{2}{3}ModuleName::{4}", (int)dbUser.Role_Id, "__", dbUser.Campaign_Id, "__", spType);
            string tagToMatch = QueryTag + string.Format("__UserId::{0}", dbUser.User_Id);

            queryStr = TagWiseQueryHandler.GetTagWiseQuery(tagToMatch, QueryTag, queryParms);
        ...
        public static dynamic /*DataTable*/ GetStakeHolderServerSideListDenormalizedNew(string from, string to, DataTableParamsModel dtModel, string commaSeperatedCampaigns, string commaSeperatedCategories, string commaSeperatedStatuses, string commaSeperatedTransferedStatus, int complaintsType, Config.StakeholderComplaintListingType listingType, string spType, int userId = -1)
        {
            DbUsers dbUser = null;
            if (userId == -1)
            {
            ...
            //----- new query code -------
            dynamic d = SetParam(dbUser, from, to, commaSeperatedCampaigns, commaSeperatedCategories, commaSeperatedStatuses, commaSeperatedTransferedStatus, dtModel, (Config.ComplaintType)complaintsType, listingType, spType);
            dynamic dListingLogic = StakeholderListingLogic.GetQueryWithParams(d);
            Dictionary<string, object> dictParams = dListingLogic.dictParam;
            string tagId = string.Format("Type::Query___Module::ComplaintsListing___ComplaintType::{0}___ListingType::{1}___CampaignId::{2}", (int)complaintsType, (int)listingType, commaSeperatedCampaigns);
            string key = string.Format("Type::Query___Module::ComplaintsListing___ComplaintType::{0}___ListingType::{1}___Action::{2}___CampaignId::{3}___UserId::{4}",
                (int)complaintsType, (int)listingType, spType, commaSeperatedCampaigns, dbUser.User_Id);


            queryStr = TagWiseQueryHandler.GetTagWiseQuery(tagId, key);
            if (!string.IsNullOrEmpty(queryStr))
            {
                //------ my new code ------
                queryStr = queryStr.Replace("@injectableWhereClause", dListingLogic.queryWhereClause);
                queryStr = queryStr.Replace("@injectableSelectClause", dListingLogic.querySelectClause);
                dictParams.Add("@fromDate", from);
                dictParams.Add("@toDate", to);
                dictParams.Add("@tableCampaignIds", DBHelper.GetSqlParamTableAgainstList("@tableCampaignIds", (List<int>)Utility.GetIntList(commaSeperatedCampaigns)));
                dictParams.Add("@tableCategoryIds", DBHelper.GetSqlParamTableAgainstList("@tableCategoryIds", (List<int>)Utility.GetIntList(commaSeperatedCategories)));
                dictParams.Add("@complaintType", complaintsType);
                dictParams.Add("@pageNo", Convert.ToInt32((d.startRow + d.endRow) / d.pageLength));
                dictParams.Add("@rowsPerPage", d.pageLength);
            ...
            {
                queryStr = StakeholderListingLogic.GetListingQuery(paramsSchoolEducation);
                dictParams = Utility.AppendDict(dictParams, dtModel.dtQueryParams);
                queryStr = StakeholderListingLogic.GetQuery(dictParams);
            }


            dynamic dToRet = DBHelper.GetDynamicList2D(queryStr, dictParams);
            return dToRet;

        }

        public static dynamic SetParam(DbUsers dbUser, string fromDate, string toDate, string campaign, string category, string complaintStatuses, string commaSeperatedTransferedStatus, DataTableParamsModel dtParams, Config.ComplaintType complaintType, Config.StakeholderComplaintListingType listingType, string spType)
        {
            dynamic dToRet = new ExpandoObject();
            dToRet.listCategories = Utility.GetIntList(campaign);
            dToRet.listTransferedStatus = Utility.GetIntList(commaSeperatedTransferedStatus); //dParam.listTransferedStatus;
            dToRet.listComplaintStatuses = Utility.GetIntList(complaintStatuses); //dParam.listComplaintStatuses;
            dToRet.whereToInject = "";

            //checkIfExistInSrcId
            //dToRet.whereOfMultisearch = dtParams.WhereOfMultiSearchParametrized;

            if (dtParams != null)
            {
                dToRet.startRow = dtParams.Start;
                dToRet.endRow = dtParams.End;
                dToRet.pageLength = dtParams.Length;
                dToRet.orderByColumnName = dtParams.ListOrder[0].columnName;
                dToRet.orderByDirection = dtParams.ListOrder[0].sortingDirectionStr;
                dToRet.whereOfMultiSearch = dtParams.WhereOfMultiSearch;
                dToRet.whereOfMultiSearchParametrized = dtParams.WhereOfMultiSearchParametrized;
                //dToRet.whereParamsDict =;
                dToRet.dictParam = dtParams.dtQueryParams;
                dToRet.whereToInject = dtParams.WhereOfMultiSearchParametrized; //dtParams.whereLogic;
            }
            else
            {
                dToRet.dictParam = new Dictionary<string, object>();
            }

            dToRet.selectToInject = "";

            dToRet.complaintType = complaintType;
            dToRet.listingType = listingType;
            int? hierarchyId = Convert.ToInt32(dbUser.Hierarchy_Id);
            int? userHierarchyId = Convert.ToInt32(dbUser.User_Hierarchy_Id);
            int? userCategoryId1 = dbUser.UserCategoryId1;
            int? userCategoryId2 = dbUser.UserCategoryId2;

            dToRet.hierarchyId = hierarchyId;
            dToRet.userHierarchyId = userHierarchyId;
            dToRet.userCategoryId1 = userCategoryId1;
            dToRet.userCategoryId2 = userCategoryId2;
            dToRet.checkIfExistInSrcId = 1;
            dToRet.checkIfExistInUserSrcId = 1;

            //bool ignoreComputedHierarchyCheck = false;//dParam.ignoreComputedHierarchyCheck;
            //int checkIfExistInSrcId = 0; //dParam.checkIfExistInSrcId;
            ...
            {
                dToRet.ignoreComputedHierarchyCheck = true;
                ...
                dToRet.whereToInject = " AND (schoolMap.Assigned_To = " + (int)Config.SchoolEducationUserSubRoles.SDU + " OR (complaints.Status_ChangedBy=" + Config.MEALoginId + "AND complaints.Complaint_Status_Id=" + (int)Config.ComplaintStatus.ResolvedVerified + ") ) ";
            }
            else if (dbUser.User_Id == Config.MEALoginId && listingType == Config.StakeholderComplaintListingType.AssignedToMe)
            {
                dToRet.hierarchyId = (int)Config.Hierarchy.None;
                //paramsModel.InnerJoinLogic = paramsModel.InnerJoinLogic + @" INNER JOIN pitb.School_Category_User_Mapping schoolMap
                //    ON complaints.Complaint_SubCategory=schoolMap.Category_Id ";
                dToRet.whereToInject = " AND (schoolMap.Assigned_To = " + (int)Config.SchoolEducationUserSubRoles.MEA + " AND complaints.Complaint_Computed_Status_Id=" + (int)Config.ComplaintStatus.ResolvedUnverified + ") ";
                dToRet.ignoreComputedHierarchyCheck = true;
                dToRet.userCategoryId1 = null;
                dToRet.userCategoryId2 = null;
                dToRet.checkIfExistInUserSrcId = 0;
            ...
            {
                List<DbPermissionsAssignment> permList = DbPermissionsAssignment.GetListOfPermissionsByTypeTypeIdAndPermissionId((int)Config.PermissionsType.User, dbUser.User_Id, (int)Config.Permissions.ViewableSchoolsToUser);

                if (permList != null && permList.Count > 0)
                {
                    var selectedPerm = permList[0];
                    var value = selectedPerm.Permission_Value;
                    dToRet.whereToInject = " AND RefField1 IN (" + value + ") ";
                }
            }
            else
            {
                dToRet.ignoreComputedHierarchyCheck = false;
            }



            dToRet.canComputeComplaintType = false;
            dToRet.canComputeCampaignId = true;
            dToRet.canComputeDateRange = true;
            dToRet.canComputeHierarchy = true;
            dToRet.canComputeUserHierarchy = true;
            dToRet.canComputeUserCategory = true;
            dToRet.canComputeStatus = true;
            dToRet.canComputeListingType = true;
            dToRet.canComputeMinMaxSrcId = true;
            dToRet.canComputeIsTransfered = true;
            dToRet.canComputeWhereOfMultisearch = true;
            dToRet.canComputeSrcId = true;
            dToRet.canComputeUserSrcId = true;


            dToRet.listUserCategory = UserCategoryModel.GetListUserCategoryModel(dbUser.ListDbUserCategory);

            dToRet.listProvinceId = Utility.GetIntList(dbUser.Province_Id);
            dToRet.listDivisionId = Utility.GetIntList(dbUser.Division_Id);
            dToRet.listDistrictId = Utility.GetIntList(dbUser.District_Id);
            dToRet.listTehsilId = Utility.GetIntList(dbUser.Tehsil_Id);
            dToRet.listUcId = Utility.GetIntList(dbUser.UnionCouncil_Id);
            dToRet.listWardId = Utility.GetIntList(dbUser.Ward_Id);
            return dToRet;
        }
        ...
        public static ListingParamsModelBase SetParamsSchoolEducation(DbUsers dbUser, string fromDate, string toDate, string campaign, string category, string complaintStatuses, string commaSeperatedTransferedStatus, DataTableParamsModel dtParams, Config.ComplaintType complaintType, Config.StakeholderComplaintListingType listingType, string spType)
        {
            ...
            if (dtParams != null)
            {
                ...
                paramsModel.WhereOfMultiSearchParametrized = dtParams.WhereOfMultiSearchParametrized;
                paramsModel.WhereParamsDict = dtParams.dtQueryParams;
            }
        ...
        public static ListingParamsModelBase SetParamsSchoolEducation_2(DbUsers dbUser, string fromDate, string toDate, string campaign, string category, string complaintStatuses, string commaSeperatedTransferedStatus, DataTableParamsModel dtParams, Config.ComplaintType complaintType, Config.StakeholderComplaintListingType listingType, string spType, List<DbPermissionsAssignment> listPermissionsAssignments)
        {
            string extraSelection = " complaints.Department_Name, complaints.Complaint_SubCategory_Name, complaints.RefField1, complaints.RefField2, complaints.RefField3, complaints.RefField4, complaints.RefField5, complaints.RefField6, complaints.Person_Cnic, complaints.Computed_Overdue_Days, complaints.Complaint_Remarks, ";
            ...
            {
                List<DbPermissionsAssignment> permList = listPermissionsAssignments.Where(w => w.Type == (int)Config.PermissionsType.User &&
                w.Type_Id == dbUser.User_Id && w.Permission_Id == (int)Config.Permissions.ViewableSchoolsToUser).ToList();
        ...
        public static VmStatusWiseComplaintsData GetAgingReportData(string dateFirst, string dateSecond, int campaignId, Config.AgingReportType agingReportType)
        {
            ...
            List<AgingReportListingData> listAgingReportData = DbComplaint.GetAgingReportListingData(campaignId, Config.ComplaintStatus.ResolvedUnverified);
        ...
        private static VmStatusWiseComplaintsData GetLowerIndividual(int userId, string startDate, string endDate, Config.UserWiseGraphType userWiseGraph, bool canDiscartHierarchyLowerThanImmediateOne)
        {
            ...
            if (!string.IsNullOrEmpty(queryStr))
            {
                ds = DBHelper.GetDataSetByQueryString(queryStr, null, 0);
                statusWiseComplaintData = GetUserStatusWiseComplaintData(ds, listVmUserWiseStatus);
                VmStatusWiseComplaintsData.SortStatusWiseData(Config.ListSchoolEducationStatuses,
        ...
        #region phcip dashboard
        public static VmPhcipEducationComplaintsData GetPhcipDashboardComplaintsStats(List<int> statusList, string startDate, string endDate)
        {
            VmPhcipEducationComplaintsData data = new VmPhcipEducationComplaintsData();
            Dictionary<string, object> paramDict = new Dictionary<string, object>();


            paramDict.Add("@StatusIds", string.Join(",", statusList).ToDbObj());
            paramDict.Add("@StartDate", startDate.ToDbObj());
            paramDict.Add("EndDate", endDate.ToDbObj());

            var stats = DBHelper.GetDataTableByStoredProcedure("sp_PCHIP_Complaints_Stats", paramDict).ToList<VmPhcipComplaintsStats>();
            data.ComplaintsStats = stats;

            return data;
        }

        public static VmPhcipEducationComplaintsData GetPhcipDashboardComplaintsSource(List<int> statusList, string startDate, string endDate)
        {
            VmPhcipEducationComplaintsData obj = new VmPhcipEducationComplaintsData();
            Dictionary<string, object> paramDict = new Dictionary<string, object>();
            ...
            var list = DBHelper.GetDataTableByStoredProcedure("sp_PCHIP_Complaints_Source", paramDict).ToList<VmPhcipComplaintsSource>();
            obj.ComplaintsSourceList = list;

            return obj;
        }

        public static VmPhcipEducationComplaintsData GetPhcipDashboardComplaintsByDistrict(List<int> statusList, string startDate, string endDate)
        {
            VmPhcipEducationComplaintsData obj = new VmPhcipEducationComplaintsData();

            VmPhcipComplaintsByDistrict data = new VmPhcipComplaintsByDistrict();
            Dictionary<string, object> paramDict = new Dictionary<string, object>();
            ...
            obj.ComplaintsByDistrict = DBHelper.GetDataTableByStoredProcedure("sp_PCHIP_Complaints_Stats_by_Districts", paramDict).ToList<VmPhcipComplaintsByDistrict>();
            return obj;
        }
        #endregion
    }
