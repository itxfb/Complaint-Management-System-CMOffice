...
namespace PITB.CMS_Common.Handler.Business
{
    public class BIStakeholderAdmin
    {
        ...
        public static VmAddStakeholderUser GetVmAddCampaigns()
        {
            VmAddStakeholderUser vms = new VmAddStakeholderUser();
            vms.ListOfCampaign = GetCampaigns(); // Get all campaigns 
            return vms;
        }
        ...
        private static List<DbCampaign> GetCampaigns()
        {
            CMSCookie cookie = AuthenticationHandler.GetCookie();
            var campaigns = cookie.Campaigns.Split(',');
            var lstCampaigns = DbCampaign.GetAllCampaign();
            return lstCampaigns;
        }
