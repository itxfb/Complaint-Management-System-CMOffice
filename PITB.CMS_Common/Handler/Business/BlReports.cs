...
using PITB.CMS_Common.Models.Custom.HtmlTable;
...
namespace PITB.CMS_Common.Handler.Business
{
    public class BlReports
    {
        ...
        public static DataTable GetDepartmentWiseCount(string from, string to, string departmentIds, string campaignIds)
        {
            //List<DbConfiguration_Assignment> pendingStatusList = DbConfiguration_Assignment.GetByTagId(Config.CONFIG_TAG_STATUS_MERGE_LIST_PENDING);
            //List<DbConfiguration_Assignment> resolvedStatusList = DbConfiguration_Assignment.GetByTagId(Config.CONFIG_TAG_STATUS_MERGE_LIST_RESOLVED);
            ...
            //}
            //make this permission base if another campaign requests this report

            DataTable data = DbComplaint.GetDepartmentListingReport(from, to, departmentIds, campaignIds);
            return data;
        }
        ...
        public static VmPhcipEducationComplaintsData TablePhcipCategoryStatReport(string datelow, string datemax)
        {
            VmStakeholderPieChart totalCount = new VmStakeholderPieChart();
            List<VmStatusWiseCount> temp = new List<VmStatusWiseCount>();
            CMSCookie cookie = new AuthenticationHandler().CmsCookie;

            List<DbStatus> statusList = BlCommon.GetStatusListByCampaignIds_Permission(Utility.GetIntList(cookie.Campaigns), cookie.UserId, cookie, Config.Permissions.StatusesForComplaintListingAll);

            return BlSchool.GetPhcipDashboardComplaintsStats(statusList.Select(s => s.Complaint_Status_Id).ToList(), datelow, datemax);
        }

        public static VmStakeholderPieChart PieChartPhcipDashboardComplaintsStatusStats(string datelow, string datemax)
        {
            VmStakeholderPieChart totalCount = new VmStakeholderPieChart();
            List<VmStatusWiseCount> temp = new List<VmStatusWiseCount>();
            ...
            List<DbStatus> statusList = BlCommon.GetStatusListByCampaignIds_Permission(Utility.GetIntList(cookie.Campaigns), cookie.UserId, cookie, Config.Permissions.StatusesForComplaintListingAll);
            var data = BlSchool.GetPhcipDashboardComplaintsStats(statusList.Select(s => s.Complaint_Status_Id).ToList(), datelow, datemax);


            if (data != null)
            {

                VmStatusWiseCount singletemp1 = new VmStatusWiseCount();
                singletemp1.name = "Resolved";
                singletemp1.y = data.TotalResolved;
                data.ColorsList.Add(data.GetColorCode(singletemp1.name));

                VmStatusWiseCount singletemp2 = new VmStatusWiseCount();
                singletemp2.name = "New";
                singletemp2.y = data.TotalNew;
                data.ColorsList.Add(data.GetColorCode(singletemp2.name));

                VmStatusWiseCount singletemp3 = new VmStatusWiseCount();
                singletemp3.name = "Overdue";
                singletemp3.y = data.TotalOverdue;
                data.ColorsList.Add(data.GetColorCode(singletemp3.name));

                //VmStatusWiseCount singletemp4 = new VmStatusWiseCount();
                //singletemp4.name = "Reopened";
                //singletemp4.y = data.TotalReopened;
                //data.ColorsList.Add(data.GetColorCode(singletemp4.name));


                VmStatusWiseCount singletemp5 = new VmStatusWiseCount();
                singletemp5.name = "Closed";
                singletemp5.y = data.TotalClosed;
                data.ColorsList.Add(data.GetColorCode(singletemp5.name));


                temp.Add(singletemp1);
                temp.Add(singletemp2);
                temp.Add(singletemp3);
                //temp.Add(singletemp4);
                temp.Add(singletemp5);

            }
            totalCount.ListStatusWiseCount = temp;
            totalCount.AllStatusColorList = data.ColorsList;
            return totalCount;
        }

        public static VmStakeholderPieChart PieChartPhcipDashboardComplaintsSource(string datelow, string datemax)
        {
            VmStakeholderPieChart totalCount = new VmStakeholderPieChart();
            List<VmStatusWiseCount> temp = new List<VmStatusWiseCount>();
            List<string> colorsList = new List<string>();
            ...
            List<DbStatus> statusList = BlCommon.GetStatusListByCampaignIds_Permission(Utility.GetIntList(cookie.Campaigns), cookie.UserId, cookie, Config.Permissions.StatusesForComplaintListingAll);
            var data = BlSchool.GetPhcipDashboardComplaintsSource(statusList.Select(s => s.Complaint_Status_Id).ToList(), datelow, datemax);
            if (data != null)
            {
                foreach (var item in data.ComplaintsSourceList)
                {
                    VmStatusWiseCount singletemp = new VmStatusWiseCount();
                    singletemp.name = item.Category;
                    singletemp.y = item.Count;
                    temp.Add(singletemp);
                    data.ColorsList.Add(data.GetColorCode(item.Category));
                }
            }
        ...
        public static VmStakeholderBarChart BarChartPhcipDashboard(string datelow, string datemax, int campaignId)
        {
            VmStakeholderBarChart chart = new VmStakeholderBarChart();
            CMSCookie cookie = new AuthenticationHandler().CmsCookie;

            List<DbStatus> statusList = BlCommon.GetStatusListByCampaignIds_Permission(Utility.GetIntList(cookie.Campaigns), cookie.UserId, cookie, Config.Permissions.StatusesForComplaintListingAll);
            VmPhcipEducationComplaintsData obj = BlSchool.GetPhcipDashboardComplaintsByDistrict(statusList.Select(s => s.Complaint_Status_Id).ToList(), datelow, datemax);
            obj.ComplaintsByDistrict = obj.ComplaintsByDistrict.OrderByDescending(o => o.Count).Take(10).ToList();


            chart.yAxisText = obj.ComplaintsByDistrict.Select(s => s.District).ToList();

            VmBarChartData data = new VmBarChartData();
            data.LegendText = "Districts";
            data.xAxisText = obj.ComplaintsByDistrict.Select(s => s.Count).ToList();

            chart.ListData.Add(data);
