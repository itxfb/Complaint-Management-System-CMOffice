...
namespace PITB.CMS_Common.Handler.DynamicFields
{
    public class DynamicFieldsHandler
    {
        ...
        public static List<DbDynamicFormControls> GetFormControls(int campaignId)
        {
            List<DbDynamicFormControls> listDbDynamicFormCtrl = DbDynamicFormControls.GetBy(campaignId, null);
            List<DbDynamicCategories> listDbDymicCategories = DbDynamicCategories.GetBy(campaignId, null);
            foreach (DbDynamicFormControls dbFc in listDbDynamicFormCtrl)
            {
                dbFc.Data = new ExpandoObject();
                if (dbFc.Control_Type == (int)Config.DynamicControlType.DropDownList ||
                    dbFc.Control_Type == (int)Config.DynamicControlType.DropDownListServerSideSearchable)
                {
                    dbFc.Data.listCategories = listDbDymicCategories.Where(n => n.CampaignId == dbFc.Campaign_Id && n.CategoryTypeId == dbFc.CategoryTypeId).ToList();
                }
            }
            return listDbDynamicFormCtrl;
        }
            ...
            foreach (DbDynamicFormControls dF in listDbDynamicForm)
            {
                switch ((Config.DynamicControlType)dF.Control_Type)
                {
                    case Config.DynamicControlType.DropDownList:
                        ...
                        tempVmDynamicFields = new VmDynamicDropDownList(dF.Id, (int)dF.Control_Type, dF.FieldName, (bool)dF.IsRequired, (int)dF.Priority, listDynamicCategories,dF.CategoryHierarchyId.HasValue? dF.CategoryHierarchyId.Value:-1,dF.Setting);
                    ...
                    case Config.DynamicControlType.CheckBox:
                        var listCats = DbDynamicCategories.GetByCampaignId((int)dF.Campaign_Id).Where(n => n.CategoryTypeId == (int)dF.CategoryTypeId).OrderBy(n => n.Name)
                              .Select(n => new VmDynamicCheckBox() { Value = n.Id.ToString(),TagId= n.TagId, Text = n.Name }).ToList();
                        tempVmDynamicFields = new VmCheckBox(dF.Id, (int)dF.Control_Type, dF.FieldName, (bool)dF.IsRequired, (int)dF.Priority, listCats);
                        listVmDynamicFields.Add(tempVmDynamicFields);
                        break;
                    ...
                    if (dictConfig.ContainsKey("isDynamic") && dictConfig["isDynamic"] == "true")
                    {
                        ...
                        else if (dictConfig["controlType"] == "dropDown" || dictConfig["controlType"] == "radio")
                        {
                            ...
                            if (dynamicField.Value != null)
                            {
                                ...
                                if (tempStrArr.Length == 1) //if only id is found
                                {
                                    DbDynamicCategories dbDc = DbDynamicCategories.GetBy(int.Parse(tempStrArr[0]));
                                    dbDf.CategoryTypeId = dbDc.Id;
                                    dbDf.FieldValue = dbDc.Name;
