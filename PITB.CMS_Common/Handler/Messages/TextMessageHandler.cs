...
namespace PITB.CMS_Common.Handler.Messages
{
    ...
    public class TextMessageHandler
    {
        ...
        public static void SendMessageOnComplaintLaunch(string phoneNo, int campaignId, int complaintId, int categoryId, string personName = null, List<VmDynamicField> listVmDynamic = null)
        {
            ...
            else if (campaignId == (int)Config.Campaign.CMCC)
            {
                string sms = String.Format("Dear Citizen,\n\nThank you for reaching out to Chief Minister Punjab Complaint Center.\n\nYour complaint has been received and your complaint no. is {0}.", complaintId);
                CallSendSmsAPI(phoneNo, sms);
            }
            else
            {
        ...
        public static void SendMessageOnStatusChange(string phoneNo, int campaignId, int complaintId, int categoryId, int statusId, string statusComments)
        {
            ...
            else if (campaignId.Equals((int)Config.Campaign.CMCC))
            {
                sms = string.Format("Dear Citizen,\n\nYour complaint no. {0} has been resolved.\n\nThank you for contacting Chief Minister Punjab's Complaint Center.", complaintId);
            }
            else
            {
