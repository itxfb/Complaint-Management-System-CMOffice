...
namespace PITB.CMS_Common.WindowServiceHandlers
{
    public class SchoolEducationHandler
    {
        ...
        public static void SycUserWiseComplaints_2()
        {
            try
            {
                DBContextHelperLinq db = new DBContextHelperLinq();
                ...
                List<DbUsers> listDbUsers = DbUsers.GetActiveUserAgainstParams(campaignId).Take(97).ToList();
                List<DbComplaint> listUserComplaints = null;
                List<DbUserWiseComplaints> listDbUserWiseComplaintsToInsert = new List<DbUserWiseComplaints>();
                ...
                var ids = listDbUsers.Select(s => s.User_Id).ToList();
                List<DbPermissionsAssignment> listOfPermissions = DbPermissionsAssignment.GetListOfPermissions((int)Config.PermissionsType.User, ids);// for specific users
                List<DbStatus> listStatuses = DbStatus.GetAll();

                List<string> listQueryBatches = new List<string>();
                int batchComputationUsersCount = 5;//13040
                int queriesPerBatch = 10;//13040
                int from = 0;
                int to = 0;
                Thread[] tl = new Thread[listQueryBatches.Count];
                int j = 0;

                var splitedUsersList = Utility.SplitList(listDbUsers, batchComputationUsersCount);

                //while (to != listDbUsers.Count)
                //{
                //    from = to;
                //    to = from + batchComputationUsersCount;
                //    to = listDbUsers.Count < to ? listDbUsers.Count : to;
                //    string query = CreateQueryBatch(listDbUsers.GetRange(from, to - from), listOfPermissions, listStatuses);
                //    listQueryBatches.Add(query);
                //}
                foreach (var item in splitedUsersList)
                {
                    string query = CreateQueryBatch(item, listOfPermissions, listStatuses);
                    listQueryBatches.Add(query);
                }


                //#region method 1
                //string qu = string.Join(";", listQueryBatches);
                //var timer_ = new Stopwatch();
                //timer_.Start();

                //var lis = ExecuteQuery(qu);

                //timer_.Stop();
                //TimeSpan timeTaken_ = timer_.Elapsed;
                //#endregion


                var queriesList = Utility.SplitList(listQueryBatches, queriesPerBatch).ToList();

                //foreach (var item in queriesList)
                //{
                //    listDbUserWiseComplaintsToInsert.AddRange(RunQueryInThreads(item, queriesPerBatch));
                //}
                Thread[] ts = new Thread[queriesPerBatch];
                for (int i = 0; i < queriesList.Count; i++)
                {
                    int temp = i;
                    ts[i] = new Thread(() =>
                    {
                        listDbUserWiseComplaintsToInsert.AddRange(RunQueryInThreads(queriesList[temp], queriesPerBatch));
                    });
                    ts[i].Start();
                }
                ...
                //from = 0;
                //to = 0;
                //while (to != listQueryBatches.Count)
                //{
                //    from = to;
                //    to = from + queriesPerBatch;
                //    to = listQueryBatches.Count < to ? listQueryBatches.Count : to;

                //    var timer = new Stopwatch();
                //    timer.Start();

                //    var tempqueries = listQueryBatches.GetRange(from, queriesPerBatch);
                //    listDbUserWiseComplaintsToInsert.AddRange(RunQueryInThreads(tempqueries, queriesPerBatch));
                ...
                ////Task[] tasks = new Task[listQueryBatches.Count];
                //List<Task> tasks = new List<Task>();
                ////for (int i = 0; i < listQueryBatches.Count; i++)
                ////{
                ////    int temp = i;
                ////    tasks[temp] = Task.Factory.StartNew(() => ExecuteQuery_Thread(listQueryBatches[temp]));
                ////}
                ////foreach (var item in listQueryBatches)
                ////{
                ////    tasks.Add(Task.Run(() =>
                ////    {
                ////        try
                ////        {
                ////            tasks.Add(Task.Factory.StartNew(() => ExecuteQuery_Thread(item)));
                ////        }
                ////        finally
                ////        {
                ////        }
                ...
                //Parallel.ForEach(listQueryBatches, item =>
                //{
                //    ExecuteQuery_Thread(item);
                //});

                //timer4.Stop();
                //TimeSpan timeTaken4 = timer4.Elapsed;

                //string foo_ = "Time taken: " + timeTaken_.ToString(@"m\:ss\.fff");
                //string foo = "Time taken: " + timeTaken.ToString(@"m\:ss\.fff");
                //string foo3 = "Time taken: " + timeTaken3.ToString(@"m\:ss\.fff");
                //string foo4 = "Time taken: " + timeTaken4.ToString(@"m\:ss\.fff");
                //#endregion

                //foreach (string query in listQueryBatches)
                //{
                //    task = new Task(() => listDbUserWiseComplaintsToInsert.AddRange(ExecuteQuery(query)));

                //    var list = DBHelper.GetDataSetByQueryString(query, null, 0).ToList<DbComplaint>();
                //    listDbUserWiseComplaintsToInsert.AddRange(
                //        UserWiseComplaintsHandler.GetUserWiseComplaintsByAgentId(list,
                //            (int)Config.Campaign.SchoolEducationEnhanced, Config.ComplaintType.Complaint,
                //            Config.StakeholderComplaintListingType.UptilMyHierarchy));
                //}

                DbUserWiseComplaints.SyncUserWiseComplaints_BulkMerge(db, listUserWiseComplaintsCurrent, listDbUserWiseComplaintsToInsert);
        ...
        public static List<DbUserWiseComplaints> RunQueryInThreads(List<string> tempqueries, int queriesPerBatch)
        {
            List<DbUserWiseComplaints> listUserWiseComplaints = new List<DbUserWiseComplaints>();
            Thread[] ts = new Thread[queriesPerBatch];
            for (int i = 0; i < tempqueries.Count; i++)
            {
                int temp = i;
                ts[i] = new Thread(() =>
                {
                    Thread.Sleep(1000);
                    Debug.WriteLine($"{Task.CurrentId} - {i} - {DateTime.Now.ToString("HH:mm:ss")}");
                    //listUserWiseComplaints.AddRange(ExecuteQuery(tempqueries[temp]));
                });
                ts[i].Start();
            }
            ...
            return listUserWiseComplaints;
        }


        //public static async Task<List<DbUserWiseComplaints>> RunWithThreads(List<string> queries, List<DbUserWiseComplaints> listDbUserWiseComplaintsToInsert)
        //{
        //    var concurrentTasks = queries.Count;
        //    var semaphore = new System.Threading.SemaphoreSlim(concurrentTasks, concurrentTasks);
        //    var tasks = new List<System.Threading.Tasks.Task<DbUserWiseComplaints>>();
        ...
        //        tasks.Add((Task<DbUserWiseComplaints>)task);
        //    }
        ...
        //        //Display($"Id: {part.PartId}, Name: {part.PartName}");
        //    }
        ...
        public static string CreateQueryBatch(List<DbUsers> listDbUsers, List<DbPermissionsAssignment> listOfPermissions, List<DbStatus> listStatuses)
        {
            string query = string.Empty;
            foreach (DbUsers dbUser in listDbUsers)
            {
                query += BlSchool.GetComplaintsAgainstUser_Query(dbUser, listOfPermissions, listStatuses,
                    Config.StakeholderComplaintListingType.AssignedToMe, Config.ComplaintType.Complaint, Config.SelectionType.Specific);
                query += ";";
                query += BlSchool.GetComplaintsAgainstUser_Query(dbUser, listOfPermissions, listStatuses,
                    Config.StakeholderComplaintListingType.UptilMyHierarchy, Config.ComplaintType.Complaint, Config.SelectionType.Specific);
                query += ";";
            }
            return query;
        }

        public static List<DbUserWiseComplaints> ExecuteQuery(string Query)
        {
            var timer = new Stopwatch();
            timer.Start();
            Debug.WriteLine($"Time Start-1-{Thread.CurrentThread.ManagedThreadId}: " + timer.Elapsed.ToString(@"m\:ss\.fff"));

            List<DbUserWiseComplaints> listDbUserWiseComplaintsToInsert = new List<DbUserWiseComplaints>();

            var list = DBHelper.GetDataSetByQueryString(Query, null, 0).ToList<DbComplaint>();

            listDbUserWiseComplaintsToInsert = UserWiseComplaintsHandler.GetUserWiseComplaintsByAgentId(list,
                (int)Config.Campaign.SchoolEducationEnhanced, Config.ComplaintType.Complaint,
                Config.StakeholderComplaintListingType.UptilMyHierarchy);
            ...
            Debug.WriteLine($"Time taken-1-{Thread.CurrentThread.ManagedThreadId}: " + timeTaken.ToString(@"m\:ss\.fff"));
            return listDbUserWiseComplaintsToInsert;
        }

        public static List<DbUserWiseComplaints> ExecuteQuery_Thread(string Query)
        {
            var timer = new Stopwatch();
            timer.Start();
            Debug.WriteLine($"Time Start-2-{Thread.CurrentThread.ManagedThreadId}: " + timer.Elapsed.ToString(@"m\:ss\.fff"));
            ...
            Debug.WriteLine($"Time taken-2-{Thread.CurrentThread.ManagedThreadId}: " + timeTaken.ToString(@"m\:ss\.fff"));
