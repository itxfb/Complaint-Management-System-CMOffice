...
//using PITB.CMS_Common.Models.DB;
using System.Text;
...
namespace PITB.CMS_Common
{
    ...
    {
        //private SelectList ToSelectList(IEnumerable<object> list, string keyName, string valueName, string defaultValue, )
        //{
        ...
        public static IEnumerable<List<T>> SplitList<T>(List<T> list, int nSize)
        {
            ...
            {
                data.Add(list.GetRange(i, Math.Min(nSize, list.Count - i)));
            }
            return data;
        }
        ...
        public static void PrintDynamic(dynamic d)
        {
            string json = JsonConvert.SerializeObject(d);
            Debug.WriteLine("------- Printing dynamic form -------");
        ...
        public static object GetPropertyValueFromDynamic(dynamic dynamic, string propertyKey)
        {
            Type objType = dynamic.GetType();
            ...
            {
                object obj;
                IDictionary<string, object> dict = (IDictionary<string, object>)dynamic;
                if (dict.TryGetValue(propertyKey, out obj))
                {
                    return obj;
                }
                return null;
            }
            return Utility.GetPropertyThroughReflection(objType, propertyKey);
            //return objType.GetProperty(property) != null;
        }

        public static void SetPropertyOfDynamic(dynamic dynamic, string propertyKey, object propertyValue)
        {
            Type objType = dynamic.GetType();
            ...
            {
                IDictionary<string, object> dict = (IDictionary<string, object>)dynamic;
                dict.Add(propertyKey, propertyValue);
            }
            //return Utility.GetPropertyThroughReflection(objType, property);
            //return objType.GetProperty(property) != null;
        }

        public static dynamic GetInitializedDynamic(Dictionary<string, object> dictKeyVal)
        {
            dynamic d = new ExpandoObject();
            if (dictKeyVal != null)
            {
                IDictionary<string, object> dict = (IDictionary<string, object>)d;
                foreach (KeyValuePair<string, object> keyVal in dictKeyVal)
                {
        ...
        public static dynamic GetInitializedDynamic(object paramsObj)
        {
            dynamic d = new ExpandoObject();
            IDictionary<string, object> dict = (IDictionary<string, object>)d;
            //foreach (object obj in paramsObj)
            //{
            Type myType = paramsObj.GetType();
            IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
            foreach (PropertyInfo prop in props)
            {
                object propValue = prop.GetValue(paramsObj, null);
        ...
        //public static dynamic GetInitializedDynamic(Dictionary<string,object> dictParam)
        //{
        //    dynamic d = new ExpandoObject();
        //    IDictionary<string, object> dict = (IDictionary<string, object>)d;
        //    //foreach (object obj in paramsObj)
        ...
        //    Type myType = paramsObj.GetType();
        //    IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
        //    foreach (KeyValuePair<string,object> prop in dictParam)
        //    {
        //        object propValue = prop.GetValue(paramsObj, null);
        //        string propName = prop.Name;
        //        dict.Add(propName, propValue);
        ...
        public static List<dynamic> GetInitializedDynamicListGeneric<T>(IEnumerable<T> listObject)
        {
        ...
        public static List<dynamic> GetInitializedDynamicList(params object[] paramsObjArr)
        {
            List<dynamic> listD = new List<dynamic>();

            foreach (object obj in paramsObjArr)
            {
                IDictionary<string, object> dict = new ExpandoObject();
                Type myType = obj.GetType();
                IList<PropertyInfo> props = new List<PropertyInfo>(myType.GetProperties());
        ...
        public static Dictionary<T, K> AppendDict<T, K>(Dictionary<T, K> dict1, Dictionary<T, K> dict2)
        {
            if (dict1 == null)
            {
                return dict2;
            ...
            //T[] keys1 = dict1.Keys.ToArray();
            T[] keys2 = dict2.Keys.ToArray();
            for (int i = 0; i < keys2.Length; i++)
            {
                dict1.Add(keys2[i], dict2[keys2[i]]);
            }
            return dict1;
        }


        public static string GetSerializedJsonWithoutSpaces(dynamic d)
        {
            return Regex.Replace(JsonConvert.SerializeObject(d), @"\s+\Z", "");
        }

        public static string SerializeDynamic(dynamic d)
        {
            return JsonConvert.SerializeObject(d, typeof(ExpandoObject), GetJsonSettings());
        }

        public static dynamic DeserializeToDynamic(string jsonStr)
        {
        ...
        #region SetDynamicResponse
        public static dynamic GetDynamicResponse(Config status, int code, string message, dynamic data)
        {
        ...
        public static dynamic GetDynamicSuccessResponse(string message = null, dynamic data = null)
        {
            dynamic resp = new ExpandoObject();
            resp.status = 1;
            resp.code = 200;
        ...
        public static dynamic GetDynamicFailureResponse(string message = null, dynamic data = null)
        {
            dynamic resp = new ExpandoObject();
            resp.status = 2;
            resp.code = 500;
        ...
        public static List<string> GetWordsList(string strInput)
        {
            string word = string.Empty;
            List<string> listStr = new List<string>();
            int spaceOccuraceCount = 0;
            bool isBetweenSingleQuotes = false;

            for (int i = 0; i < strInput.Length; i++)
            {
                bool hasSpaceOccured = false;

                if (strInput[i] == '\'')
                {
                    if (isBetweenSingleQuotes == false)
                    {
                        isBetweenSingleQuotes = true;
                    }
                    else if (isBetweenSingleQuotes == true)
                    {
                ...
                if (!isBetweenSingleQuotes && (strInput[i] == ' '
                    || strInput[i] == '\r'
                    || strInput[i] == '\n'
                    || strInput[i] == '\t'))
                {
                    ...
                    spaceOccuraceCount++;
                    hasSpaceOccured = true;
                    word = string.Empty;
                }
                else
                {
                    spaceOccuraceCount = 0;
                    hasSpaceOccured = false;
                }

                // it means first character has occured
                if (spaceOccuraceCount == 0 && !hasSpaceOccured)
                {
                    word += strInput[i];
                }
        ...
        public static string ReplaceStringWithDict(string strInput, Dictionary<string, string> dict)
        {
            ...
            {
                //string strToReplace = string.Format("{{{0}}}", d.Key);
                strInput = strInput.Replace(d.Key, d.Value);
            }
            return strInput;
        }
        ...
        public static PITB.CMS_Common.Config.CaptchaResponse ValidateCaptcha(string response)
        {
            ...
            return JsonConvert.DeserializeObject<PITB.CMS_Common.Config.CaptchaResponse>(jsonResult.ToString());
        }
        // GET: /CRUD/Edit/5  
        ...
        public static string GetHierarchyValueName(Config.Hierarchy hierarchyId, int hierarchyVal)
        {
            ...
            switch (hierarchyId)
            {
                case Config.Hierarchy.Province:
                    DbProvince dbProvince = DbProvince.GetById(hierarchyVal);
                    if (dbProvince == null)
                    {
                        strVal = "N/A";
                    }
                    ...
                    {
                        strVal = dbProvince.Province_Name;
                    }
                    return strVal;
                ...
                case Config.Hierarchy.Division:
                    DbDivision dbDivision = DbDivision.GetById(hierarchyVal);
                    ...
                    {
                        strVal = dbDivision.Division_Name;
                    }
                    return strVal;
                ...
                case Config.Hierarchy.District:
                    ...
                    DbDistrict dbDistrict = DbDistrict.GetById(hierarchyVal);
                    if (dbDistrict == null)
                    {
                        strVal = "N/A";
                    }
                    ...
                    {
                        strVal = dbDistrict.District_Name;
                    }
                    return strVal;
                    break;

                case Config.Hierarchy.Tehsil:
                    DbTehsil dbTehsil = DbTehsil.GetById(hierarchyVal);
                    if (dbTehsil == null)
                    {
                        strVal = "N/A";
                    }
                    ...
                    {
                        strVal = dbTehsil.Tehsil_Name;
                    }
                    return strVal;
                    break;

                case Config.Hierarchy.UnionCouncil:
                    DbUnionCouncils dbUc = DbUnionCouncils.GetById(hierarchyVal);
                    if (dbUc == null)
                    {
                        strVal = "N/A";
                    }
                    ...
                    {
                        strVal = dbUc.Councils_Name;
                    }
                    return strVal;
                    break;

                case Config.Hierarchy.Ward:
                    DbWards dbWards = DbWards.GetByWardId(hierarchyVal);
