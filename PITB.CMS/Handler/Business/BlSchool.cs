    ...
    public class BlSchool
    {
        ...
        private static void PrepareMessageToStakeholderSupervisor(DbComplaint dbComplaint, int campaignId, DbComplaintType dbComplaintType, DbComplaintSubType dbComplaintSubType, DbSchoolsMapping dbSchools)
        {
            ...
            if (listDbUsers.Count > 0)
            {
                ...
                int hierarchyVal = Convert.ToInt32(DbComplaint.GetHierarchyIdValueAgainstHierarchyId((CMS.Config.Hierarchy)dbComplaint.Complaint_Computed_Hierarchy_Id, dbComplaint));
                string hierarchyString = CMS.Utility.GetHierarchyValueName((CMS.Config.Hierarchy)dbComplaint.Complaint_Computed_Hierarchy_Id, hierarchyVal);
        ...
        public static List<DbComplaint> GetComplaintsAgainstUser(DbUsers dbUser, Config.StakeholderComplaintListingType listingType, Config.ComplaintType complaintType, CMS.Config.SelectionType selectionType)
        {
            ...
            if (selectionType == CMS.Config.SelectionType.All)
            {
        ...
        public static void SendMessageToSEAllUsers()
        {
            try
            {
                ...
                for (int i = 0; i < listDbUsers.Count; i++)
                {
                    ...
                    smsModel = new SmsModel((int)CMS.Config.Campaign.SchoolEducationEnhanced, listDbUsers[i].Phone, messageStr,
                    (int)Config.MsgType.ToStakeholder,
                    (int)Config.MsgSrcType.Web, DateTime.Now, 1, (int)Config.MsgTags.SchoolEducationStakeholderUsernameChangeReminder);
                ...
                if (lstUsers != null)
                {
                    ...
                    foreach (DbUsers user in lstSortedUsers)
                    {
                        ...
                        if (!String.IsNullOrEmpty(user.Phone) && !String.IsNullOrEmpty(user.Password))
                        {
                            ...
                            SmsModel smsModel = null;
                            smsModel = new SmsModel((int)CMS.Config.Campaign.SchoolEducationEnhanced, user.Phone, messageStrWithoutUsername,
                            (int)Config.MsgType.ToStakeholder,
                            (int)Config.MsgSrcType.Web, DateTime.Now, 1, (int)Config.MsgTags.SchoolEducationStakeholderUsernameChangeReminder);
                            new Thread(delegate()
                            {
