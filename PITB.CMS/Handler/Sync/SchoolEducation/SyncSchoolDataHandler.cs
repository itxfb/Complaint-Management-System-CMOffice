...
using PITB.CMS.Models.API.Request.SchoolEducation;
...
namespace PITB.CMS.Handler.Sync.SchoolEducation
{
    public class SyncSchoolDataHandler
    {
        ...
        public static void SynRegionAndShoolsDataMain(DateTime from, DateTime to)
        {
            try
            {
                ...
                List<DbCrmIdsMappingToOtherSystem> listCrmIdsMapping = DbCrmIdsMappingToOtherSystem.GetAll();

                List<DbProvince> listProvince = DbProvince.AllProvincesList();

                List<DbDivision> listDivision = DbDivision.GetByProvinceIdsStrAndGroupId(Utility.GetCommaSepStrFromList(listProvince.Select(n=>n.Province_Id).ToList()),null);

                List<DbDistrict> listDistricts = DbDistrict.GetAll();

                int? groupId = DbHierarchyCampaignGroupMapping.GetModelByCampaignId((int) Config.Campaign.SchoolEducationEnhanced, Config.Hierarchy.Tehsil);
                List<DbTehsil> listTehsils = DbTehsil.GetByDistrictIdList(Utility.GetNullableIntList(listDistricts.Select(n => n.District_Id).ToList()), groupId);

                groupId = DbHierarchyCampaignGroupMapping.GetModelByCampaignId((int)Config.Campaign.SchoolEducationEnhanced, Config.Hierarchy.UnionCouncil);
                List<DbUnionCouncils> listUnionCouncils = DbUnionCouncils.GetUnionCouncilList(Utility.GetNullableIntList(listTehsils.Select(n => n.Tehsil_Id).ToList()), groupId);

                List<DbSchoolsMapping> lisDbSchoolMapping = DbSchoolsMapping.GeAll();
                ...
                for (int i=1; i < listDataRequired.Count; i++)
                {
                    try
                    {
                        ...
                        else if (listDataRequired[i] == Config.SyncApiRequiredData.tehsils)
                        {
                            ...
                            //--------------------Sync Region and School Data main-------------------------
                            DbWindowServiceError.SaveErrorLog(2, "Start SyncRegionAndSchoolDataMain 5", "Start SchoolEducationHandler.SynRegionAndShoolsDataMain() 5", Config.ServiceType.SyncSchoolDataMain);
                            //---------------------------------------------
                        ...
                        else if (listDataRequired[i] == Config.SyncApiRequiredData.school)
                        {
                            ...
                            listDistricts = DbDistrict.GetAll();

                            groupId = DbHierarchyCampaignGroupMapping.GetModelByCampaignId((int)Config.Campaign.SchoolEducationEnhanced, Config.Hierarchy.Tehsil);
                            listTehsils = DbTehsil.GetByDistrictIdList(Utility.GetNullableIntList(listDistricts.Select(n => n.District_Id).ToList()), groupId);

                            groupId = DbHierarchyCampaignGroupMapping.GetModelByCampaignId((int)Config.Campaign.SchoolEducationEnhanced, Config.Hierarchy.UnionCouncil);
                            listUnionCouncils = DbUnionCouncils.GetUnionCouncilList(Utility.GetNullableIntList(listTehsils.Select(n => n.Tehsil_Id).ToList()), groupId);

                            List<DbComplaint> listDbComplaint = DbComplaint.GetBy((int)CMS.Config.Campaign.SchoolEducationEnhanced, new List<int> { (int)Config.ComplaintStatus.PendingFresh, (int)Config.ComplaintStatus.PendingReopened, (int)Config.ComplaintStatus.UnsatisfactoryClosed });
                ...
                int campaignId = (int) PITB.CMS.Config.Campaign.SchoolEducationEnhanced;
                List<DbUsers> listDbUsers = DbUsers.GetUsersAgainstCampaign(campaignId);
            ...
            foreach (Pair<ApiResponseSyncSEModel.ResponseSEDataDistrict.District, DbCrmIdsMappingToOtherSystem, DbDistrict, Config.SyncOperations> overallMap in listOverallMapping)
            {
                try
                {
                    ...
                    else if (overallMap.Item4 == Config.SyncOperations.Add)
                    {
                        ...
                        DbDistrict dbDistrict = DbDistrict.GetDbDistrict(overallMap.Item1);
               ...
               // SqlConnection connection = new SqlConnection(conStr);
                ...
                DbDistrict.BulkMerge(listToMerge, connection);
                ...
                {
                    dbCrmIdsMappingToOtherSystem = DbCrmIdsMappingToOtherSystem.GetModel(dbTempCrmIdsMapping,
                ...
                DbCrmIdsMappingToOtherSystem.BulkMerge(listDbCrmMappingToOtherSystem, connection);
                connection.Close();
            }
            catch (Exception)
            {
        ...
        public static bool SynTehsils(/*DBContextHelperLinq db,*/ List<DbCrmIdsMappingToOtherSystem> listCrmIdsMapping, List<DbTehsil> listDbTehsils, ApiResponseSyncSEModel.ResponseSEDataTehsil responseTehsil)
        {
            ...
            List<DbCrmIdsMappingToOtherSystem> listCrmDistrictIdsMapping =
                DbCrmIdsMappingToOtherSystem.Get(dbTempCrmIdsMapping.Crm_Module_Id, (int)Config.Hierarchy.District,
            ...
            foreach (Pair<ApiResponseSyncSEModel.ResponseSEDataTehsil.Tehsil, DbCrmIdsMappingToOtherSystem, DbTehsil, Config.SyncOperations> overallMap in listOverallMapping)
            {
                try
                {
                    ...
                    {
                        //---- add value in main table
                        DbTehsil dbTehsil = DbTehsil.GetDbTeshil(overallMap.Item1,
            ...
            ////---------------------------------------------
                ...
                listToMerge.AddRange(listModelToAdd.Select(x => x.Item1).ToList());
                DbTehsil.BulkMerge(listToMerge, connection);
            ...
            {
                DbWindowServiceError.SaveErrorLog(1, Environment.StackTrace, ex, CMS.Config.ServiceType.SyncSchoolDataMain);
                throw;
            }
            finally
            {
        ...
        public static bool SynUnionCouncils(/*DBContextHelperLinq db,*/ List<DbCrmIdsMappingToOtherSystem> listCrmIdsMapping, List<DbUnionCouncils> listDbUc, ApiResponseSyncSEModel.ResponseSEDataMarkaz responseMarkaz)
        {
            ...
            try
            {
                ...
                List<DbCrmIdsMappingToOtherSystem> listCrmTehsilIdsMapping =
                    DbCrmIdsMappingToOtherSystem.Get(dbTempCrmIdsMapping.Crm_Module_Id, (int) Config.Hierarchy.Tehsil,
                        dbTempCrmIdsMapping.OTS_System_Id, dbTempCrmIdsMapping.OTS_Module_Id);
                            ...
                            DbUnionCouncils, Config.SyncOperations> overallMap in listOverallMapping)
                {
                    try
                    {
                        ...
                        {
                            //---- add value in main table
                            DbUnionCouncils dbUc = DbUnionCouncils.GetDbMarkaz(overallMap.Item1,
                ...
                //}
                DbUnionCouncils.BulkMerge(listUc, connection);
            ...
            ApiResponseSyncSEModel.ResponseSEDataSchool responseSchoolMap, List<DbComplaint> listDbComplaint,List<DbProvince> listProvinces, List<DbDivision> listDivisions, List<DbDistrict> listDistricts, List<DbTehsil> listTehsils, List<DbUnionCouncils> listUc)
        {
            ...
            try
            {
                ...
                //
                List<DbSchoolEducationHeadMapping> listDbHeadMapping = DbSchoolEducationHeadMapping.GetAll();
                ...
                foreach (Pair<ApiResponseSyncSEModel.ResponseSEDataSchool.School, DbCrmIdsMappingToOtherSystem, DbSchoolsMapping, Config.SyncOperations> overallMap in listOverallMapping)
                {
                    try
                    {
                        ...
                        else if (overallMap.Item4 == Config.SyncOperations.Add)
                        {
                            ...
                            {
                                //---- add value in main table
                                dbSchoolMap = DbSchoolsMapping.GetDbSchoolMapping( overallMap.Item1, listHierarchyIdsMapping,
                                    listDistricts,
                                    listTehsils, listUc);
                ...
                DbSchoolsMapping.BulkMerge(listSchoolMap, connection);
                DbSchoolEducationHeadMapping.BulkMerge(listHeadMap, connection);
                ...
                DbComplaint.BulkMerge(listDbSchoolEducationComplaints.Select(n=>n.DbComplaint).ToList(), connection);
                connection.Close();
            }
            catch (Exception ex)
            {
            ...
            List<DbProvince> listProvinces, List<DbDivision> listDivisions, List<DbDistrict> listDistricts, List<DbTehsil> listTehsils, List<DbUnionCouncils> listUc)
        {
            ...
            try
            {
                ...
                List<DbCrmIdsMappingToOtherSystem> listHierarchyMap =
                    DbCrmIdsMappingToOtherSystem.Get((int)Config.CrmModule.Hierarchy, dbTempCrmIdsMapping.OTS_System_Id, (int)Config.CrmModule.Hierarchy);
                ...
                List<DbComplaintType> listDbComplaintType =
                    DbComplaintType.GetByDepartmentIds(Utility.GetNullableIntList(DbDepartment.GetByCampaignId((int) Config.Campaign.SchoolEducationEnhanced)
               ...
               // int? provinceId, divisionId, districtId, tehsilId, ucId; 
                    ...
                    try
                    {
                        ...
                        else if (overallMap.Item4 == Config.SyncOperations.Add)
                        {
                            try
                            {
                                ...
                                //---- add value in main table
                                DbUsers dbUser = DbUsers.GetDbUser(overallMap.Item1, listHierarchyMap,
                ...
                DbUsers.BulkMerge(listDbUsersToMerge, connection, false); // is updating
                
                List<DbUsers> listDbUsersToAdd = listModelToAdd.Select(x => x.Item1).ToList();
                listDbUsersToMerge.AddRange(listDbUsersToAdd);
                DbUsers.BulkMerge(listDbUsersToAdd, connection, true); // is adding
