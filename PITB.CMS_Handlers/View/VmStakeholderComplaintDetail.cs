...
using PITB.CMS_Models.Custom.CustomForm;
...
namespace PITB.CMS_Handlers.View
{
    public class VmStakeholderComplaintDetail
    {
        ...
        public VmStakeholderComplaintDetail GetComplaintDetail(DbComplaint dbComplaint, List<DbDynamicComplaintFields> ListDynamicComplaintFields, VmStakeholderComplaintDetail.DetailType detailType)
        {
            ...
            int hierarchyVal = RepoDbComplaint.GetHierarchyIdValueAgainstComplaint(dbComplaint);
            ...
            List<DbUsers> listDbUser = RepoDbUsers.GetResponsibleOfficialList(dbComplaint);
            ...
            if (dbComplaint.Dt1 == null && (listDbUser == null || listDbUser.Count==0))
            {
                ...
                listDbUser.Add(RepoDbUsers.GetUser(new AuthenticationHandler().CmsCookie.UserId));
            }
            
            
            if (listDbUser != null && listDbUser.Count>0)
            {
                this.dbUserStakeholder = listDbUser.First();
            ...
            if (this.currDetailType == VmStakeholderComplaintDetail.DetailType.AssignedToMe && PermissionHandler.IsPermissionAllowedInUser(Config.Permissions.StakeholderStatusesOnStatusChangeView)) // status getting from permission
            {
            ...
            else if (this.currDetailType == VmStakeholderComplaintDetail.DetailType.All && PermissionHandler.IsPermissionAllowedInUser(Config.Permissions.ShowStatusChangeInComplaintsAllStakeholder)) // status getting from permission
            {
                //CMSCookie cookie = AuthenticationHandler.GetCookie();
                DbPermissionsAssignment dbPermissionAssignment = DbPermissionsAssignment.GetListOfPermissions
