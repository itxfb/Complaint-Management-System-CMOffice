...
namespace PITB.CMS_Handlers.StakeHolder
{
    public class SchoolEducationUsersHandler
    {
        ...
        public static void DumpUserWiseSupervisorMapping(int hierarchyId, int campaignId, int maxHierarchy)
        {
            try
            {
                RepoDbUserWiseSupervisorMapping.BulkMerge(new List<DbUserWiseSupervisorMapping>());
                ...
                RepoDbUserWiseSupervisorMapping.DeleteEntries(campaignId);
                RepoDbUserWiseSupervisorMapping.BulkMerge(listDbUserWiseSupervisorMappings);
        ...
        public static List<DbUserWiseSupervisorMapping> GetUserWiseSupervisorMappings(int hierarchyId, int campaignId, int maxHierarchy)
        {
            ...
            List<DbUsers> listDbUsers = RepoDbUsers.GetActiveUserAgainstParams(campaignId);
        ...
        {
            List<DbUsers> listDbUsersLowerTemp = UsersHandler.FindUserLowerThanCurrentHierarchy2(dbUser.User_Id, Utility.GetIntByCommaSepStr(dbUser.Campaigns),
                 (Config.Hierarchy)dbUser.Hierarchy_Id, Utility.GetIntByCommaSepStr(RepoDbUsers.GetHierarchyIdValueAgainstHierarchyId(dbUser)), true, listDbUsers,null);
