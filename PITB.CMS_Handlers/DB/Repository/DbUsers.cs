    ...
    public class RepoDbUsers
    {
        ...
        public static string GetHierarchyVal(DbUsers dbUser)
        {
            return Utility.GetHierarchyValueName((Config.Hierarchy)dbUser.Hierarchy_Id,
                Utility.GetIntByCommaSepStr(RepoDbUsers.GetHierarchy((Config.Hierarchy)dbUser.Hierarchy_Id, dbUser)));
        }
        ...
        public static List<DbUsers> GetResponsibleOfficialList(DbComplaint dbComplaint)
        {
            ...
            {
                int? escalationStatus = RepoDbStatus.GetById((int)dbComplaint.Complaint_Status_Id).EscalationStatus;
                if (escalationStatus != null)
                {
                    if (escalationStatus == 0) //If no escalation is provided
                    {
                        ...
                        listDbUser.Add(RepoDbUsers.GetUser((int)dbComplaint.Status_ChangedBy));
            ...
            if (!isValuesAssigned) // is value assigned
            {
                if (isOverdue)// if minimum level/overdue
                {
                    if (dbComplaint.SrcId1 != null) //If min hierarchy exist
                    {
                        ...
                        hierarchyIdVal = RepoDbComplaint.GetHierarchyIdValueAgainstComplaint(dbComplaint,
                ...
                listDbUser = RepoDbUsers.GetByCampIdH_IdUserH_Id2(Convert.ToInt32(dbComplaint.Compaign_Id), hierarchyId,
        ...
        public static List<DbUsers> GetByCampIdH_IdUserH_Id2(int campaignId, Config.Hierarchy hierarchyId, int hierarchyIdVal, int? userHierarchyId, int categoryId, int? userCatogoryId1, int? userCategoryId2)
        {
            try
            {
                ...
                List<DbUserCategory> listComplaintDbUserCategory = RepoDbUserCategory.GetuserCategoryList(userCatogoryId1, userCategoryId2);
        ...
        public static bool BulkMerge(List<DbUsers> listToMerge, SqlConnection con, bool isAdding)
        {
            ...
            */
            ...
            if (isAdding)
            {
                ...
                RepoDbUserCategory.BulkMerge(listDbUserCategory, con);
                RepoDbPermissionsAssignment.BulkMerge(listDbPermissionsAssignment, con);
            }
            return true;
        }
