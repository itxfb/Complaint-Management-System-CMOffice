...
namespace PITB.CMS_Handlers.DB.Repository
{
    ...
    public class RepoDbDivision
    {
        ...
        public static IEnumerable<SelectListItem> GetSelectListofDivisionForCampaignAndProvinceIds(int campaignId, int provinceId)
        {
            try
            {
                int groupId = RepoDbHierarchyCampaignGroupMapping.GetGroupIdForCampaignAndHierarchyIds(campaignId, (int)Config.Hierarchy.Province);
                List<DbDivision> lstDiv = null;
                if (groupId == -1)
                {
                    lstDiv = RepoDbDivision.GetByProvinceAndGroupId(provinceId, null);
                }
                else
                {
                    lstDiv = RepoDbDivision.GetByProvinceAndGroupId(provinceId, groupId);
                }
