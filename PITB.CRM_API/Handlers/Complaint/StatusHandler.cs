...
namespace PITB.CRM_API.Handlers.Complaint
{
    public class StatusHandler
    {
        public static ApiStatus ChangeStatus(string username, int complaintId, int statusId, string statusComments, List<Picture> listPictures, Int64 apiRequestId, bool canSendMessage = true)
        {
            ...
            List<PITB.CMS.Models.Custom.AssignmentModel> assignmentModelList =
                PITB.CMS.Handler.Complaint.Assignment.AssignmentHandler.GetAssignmentModelOnStatusChange2(Convert.ToInt32(dbUser.Hierarchy_Id), Convert.ToInt32(dbUser.User_Hierarchy_Id), dbComplaint,
                    statusId, DateTime.Now, PITB.CMS.Models.DB.DbAssignmentMatrix.GetByCampaignIdAndCategoryId(campaignId, categoryId, subcategoryId), catRetainingHours, canResetOnPendingReopen);
