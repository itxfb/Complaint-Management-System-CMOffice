...
using PITB.CMS;
using PITB.CMS.Handler.API;
using PITB.CMS.Models.Custom.Police;
...
using PITB.CRM_API.Handlers.Location;
using PITB.CRM_API.Handlers.Social;
...
using DbCampaign = PITB.CRM_API.Models.DB.DbCampaign;
using DbComplaint = PITB.CRM_API.Models.DB.DbComplaint;
using DbComplaintStatusChangeLog = PITB.CRM_API.Models.DB.DbComplaintStatusChangeLog;
using DbComplaintSubType = PITB.CRM_API.Models.DB.DbComplaintSubType;
using DbComplaintType = PITB.CRM_API.Models.DB.DbComplaintType;
using DbComplaintVote = PITB.CRM_API.Models.DB.DbComplaintVote;
using DbDistrict = PITB.CRM_API.Models.DB.DbDistrict;
using DbMobileRequest = PITB.CRM_API.Models.DB.DbMobileRequest;
...
using DbProvince = PITB.CRM_API.Models.DB.DbProvince;
using DbTehsil = PITB.CRM_API.Models.DB.DbTehsil;
using DbUnionCouncils = PITB.CRM_API.Models.DB.DbUnionCouncils;
using DbUsers = PITB.CRM_API.Models.DB.DbUsers;
using DbWards = PITB.CRM_API.Models.DB.DbWards;
    ...
    public class BlLwmcApiHandler
    {
        ...
        public static ComplaintSubmitResponse SubmitComplaint(SubmitLWMCComplaintModel submitComplaintModel, Int64 apiRequestId, int appId)
        {
            try
            {
                ...
                */
                ...
                new Thread(delegate()
                {
                    PITB.CMS.Handler.Messages.TextMessageHandler.PrepareAndSendMessageToStakeholdersOnComplaintLaunch(PITB.CMS.Models.DB.DbComplaint.GetByComplaintId(complaintId));
                }).Start();

                // Send notification to users
                PITB.CMS.Models.DB.DbComplaint dbComplaint = PITB.CMS.Models.DB.DbComplaint.GetByComplaintId(complaintId);
                int hierarchyVal = PITB.CMS.Models.DB.DbComplaint.GetHierarchyIdValueAgainstComplaint(dbComplaint);
                PITB.CMS.Config.Hierarchy hierarchyId = (PITB.CMS.Config.Hierarchy)dbComplaint.Complaint_Computed_Hierarchy_Id;
                int? userHierarchyId = dbComplaint.Complaint_Computed_User_Hierarchy_Id;
                List<PITB.CMS.Models.DB.DbUsers> listDbUser = PITB.CMS.Models.DB.DbUsers.GetByCampIdH_IdUserH_Id2(Convert.ToInt32(dbComplaint.Compaign_Id), hierarchyId,
                ...
                foreach (PITB.CMS.Models.DB.DbUsers dbUsers in listDbUser)
                {
        ...
        public static GetComplainantComplaintModel GetComplainantComplaints(SubmitGetComplaintModel model)
        {
            ...
            foreach (DbComplaint dbComplaint in listDbComplaints)
            {
                ...
                {
                    provinceName = dbComplaint.Province_Name,
                    districtName = dbComplaint.District_Name,
                    tehsilName = dbComplaint.Tehsil_Name,
                    ucName = dbComplaint.UnionCouncil_Name,
                    LocationArea = dbComplaint.LocationArea,

                    ExternalUserId = dbComplaint.Person_External_User_Id,
                    ...
                    campaignName = DbCampaign.GetById(Convert.ToInt32(dbComplaint.Compaign_Id)).Campaign_Name,
                    ...
                    categoryName = dbComplaint.Complaint_Category_Name,
                    subCategoryID = Convert.ToInt32(dbComplaint.Complaint_SubCategory),
                    subCategoryName = dbComplaint.Complaint_SubCategory_Name,
                    ...
                    UpVotes = Convert.ToInt32(dbComplaint.Vote_Up_Count),
                    DownVotes = Convert.ToInt32(dbComplaint.Vote_Down_Count),
                    UserVote = userVote,
                    ComplainantRemarks = dbComplaint.Complainant_Remark_Str,
                    StakeHolderRemarks = dbComplaint.StatusChangedComments,
                    StakeHolderRemarksDate = (dbComplaint.StatusChangedDate_Time == null) ? string.Empty : dbComplaint.StatusChangedDate_Time.ToString(),
                    ComplainantSatisfactionStatus = (dbComplaint.Complainant_Remark_Id == null) ? (int)Config.ComplainantRemarkType.None : (int)((Config.ComplainantRemarkType)Convert.ToInt32(dbComplaint.Complainant_Remark_Id)),
                    DistanceFromYourLocation = (dbComplaint.Latitude.HasValue) ? GetDistanceBetweenTwoPoints(model.Latitude, model.Longitude, dbComplaint.Latitude.Value, dbComplaint.Longitude.Value, 'K') : -1,
                    ComputedRemainingTotalTime = dbComplaint.Computed_Remaining_Total_Time
        ...
        public static SyncModel SubmitComplainantLogin(SubmitComplainantLogin submitComplainantLogin, int appId, Config.PlatformID platformId, Config.Language language, int dbVersionId)
        {
            ...
            else
            {
                ...
                {
                    decimal cnic = DbUniqueIncrementor.GetUniqueValue(CMS.Config.UniqueIncrementorTag.Cnic);
                    string cnicStr = cnic.ToString();
                    
                    //cnicStr = CMS.Utility.PaddLeft(cnicStr, CMS.Config.CnicLength, '0');
                    submitComplainantLogin.Cnic= CMS.Utility.PaddLeft(cnicStr, CMS.Config.CnicLength, '0');
        ...
        public static LwmcResponseStakeholderLogin SubmitStakeholderLoginWithPhoneNo(SubmitStakeHolderLogin submitStakeHolderLogin, Config.PlatformID platformId)
        {
            ...
            if (isUsernamePresent) // username and cnic present
            {
                ...
                {
                    CMS.Models.DB.DbUsers dbUsers = CMS.Models.DB.DbUsers.GetUser(submitStakeHolderLogin.Username,
