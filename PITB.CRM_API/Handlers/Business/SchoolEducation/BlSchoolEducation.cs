...
using PITB.CRM_API.Models.API.SchoolEducation.Sync.Response;
using PITB.CMS.Models.DB;
using DbUsers = PITB.CMS.Models.DB.DbUsers;
    ...
    public class BlSchoolEducation
    {
        public static dynamic SubmitStakeholderLogin(dynamic d)
        {
            ...
            if (isUsernamePresent) // username and cnic present
            {
                ...
                response = PITB.CMS.Utility.GetApiResponse(true, "User present", null , response);
                return response;
            }
            else // if username not present
            {
                response = PITB.CMS.Utility.GetApiResponse(true, "User not found", null, response);
                return response;
            }
        ...
        public static GetSchoolEducationComplaintModel GetSchoolComplaintsForMea(SchoolApiModel schoolApiModel)
        {
            ...
            List<PITB.CRM_API.Models.DB.DbComplaint> listDbComplaints = PITB.CRM_API.Models.DB.DbComplaint.GetByTableRef(CMS.Config.TableRef.SchoolEducation, Utility.GetNullableIntList(listDbSchools.Select(n => n.Id).ToList()));
            ...
            List<PITB.CRM_API.Models.DB.DbAttachments> listDbAttachment = new List<PITB.CRM_API.Models.DB.DbAttachments>();

            getComlaintsModel.TotalComplaints = listDbComplaints.Count;
            foreach (PITB.CRM_API.Models.DB.DbComplaint dbComplaint in listDbComplaints)
            {
                //listAttachments.Clear();
                listChangeStatusLogIds = PITB.CRM_API.Models.DB.DbComplaintStatusChangeLog.GetActiveStatusChangeLogIdsListAgainstComplaintId(dbComplaint.Id);
                foreach (int changeLogId in listChangeStatusLogIds) //  get all attachment against change log ids
                {
                    listDbAttachment.AddRange(PITB.CRM_API.Models.DB.DbAttachments.GetByRefAndComplaintId(dbComplaint.Id, Config.AttachmentReferenceType.ChangeStatus, changeLogId));
                }
                ...
                listDbAttachment = PITB.CRM_API.Models.DB.DbAttachments.GetByRefAndComplaintIdAndFileType(dbComplaint.Id, Config.AttachmentReferenceType.Add, dbComplaint.Id, Config.FileType.File);
                ...
                listDbAttachment = PITB.CRM_API.Models.DB.DbAttachments.GetByRefAndComplaintIdAndFileType(dbComplaint.Id, Config.AttachmentReferenceType.Add, dbComplaint.Id, Config.FileType.Video);
        ...
        public static dynamic GetPrivateComplaints(dynamic reqBody, HttpRequest request)
        {
            ...
            try
            {
                ...
                List<CMS.Models.DB.DbComplaint> listDbComplaints = CMS.Models.DB.DbComplaint.GetByDateRange((int)Config.Campaign.SchoolEducationEnhanced, 2, fromDate, toDate);
                ...
                resp = PITB.CMS.Utility.GetApiResponse(true, null, null, resp);
            }
            catch (Exception ex)
            {
                resp = PITB.CMS.Utility.GetApiResponse(false, null, null, resp);
            }
            return resp;
        }
        ...
        public static dynamic GetETransferComplaints(dynamic reqBody, HttpRequest request)
        {
            ...
            try
            {
                ...
                List<CMS.Models.DB.DbComplaint> listDbComplaints = null;
                if(fromDate!=null && toDate!=null && !string.IsNullOrEmpty(cnic)) // if from to date and cnic exist
                {
                    listDbComplaints = CMS.Models.DB.DbComplaint.GetByDateRangeAndNotOfRefField((int)Config.Campaign.SchoolEducationEnhanced, CMS.Config.ComplaintType.Complaint, 2, (DateTime)fromDate, (DateTime)toDate, cnic);
                }
                else if (fromDate != null && toDate != null && string.IsNullOrEmpty(cnic)) // if only date exist
                {
                    listDbComplaints = CMS.Models.DB.DbComplaint.GetByDateRangeAndNotOfRefField((int)Config.Campaign.SchoolEducationEnhanced, CMS.Config.ComplaintType.Complaint, 2, (DateTime)fromDate, (DateTime)toDate);
                }
                else if(fromDate == null && toDate == null && !string.IsNullOrEmpty(cnic)) // if only cnic exist
                {
                    listDbComplaints = CMS.Models.DB.DbComplaint.GetByCnicAndNotOfRefField((int)Config.Campaign.SchoolEducationEnhanced, CMS.Config.ComplaintType.Complaint, 2, cnic);
                }
        ...
        public static ComplaintSubmitResponseSE SubmitComplaint(SubmitSEComplaintModel submitComplaintModel, Int64 apiRequestId, int appId)
        {
            try
            {
                ...
                d.sTag.t1 = "PublicSchools"; d.sTag.t2 = "eTransferSIS"; d.sTag.t3 = "PrivateSchools";
                d.sbTag = new ExpandoObject();
                d.sbTag.t1 = new List<string> { d.sTag.t1, d.sTag.t2 };
                d.sbTag.t2 = new List<string> { d.sTag.t3 };
                ...
                if (submitComplaintModel.tagId == d.sTag.t2) //if 
                {
                    isPmiuRegion = true;
                ...
                PITB.CRM_API.Models.DB.DbPersonInformation dbPersonInfo = PITB.CRM_API.Models.DB.DbPersonInformation.GetByCnic(submitComplaintModel.cnic);
                ...
                else if(app == Config.AppID.SmartLahore)
                {
                    ...
                    divisionId = Convert.ToInt32(PITB.CRM_API.Models.DB.DbDistrict.GetById((int)submitComplaintModel.districtID).Division_Id);
                    ...
                    paramDict.Add("@Division_Id", PITB.CRM_API.Models.DB.DbDistrict.GetById((int)submitComplaintModel.districtID).Division_Id.ToDbObj());
                ...
                divisionId = PITB.CRM_API.Models.DB.DbDistrict.GetById(Convert.ToInt32(dbSchools.System_District_Id)).Division_Id;
                provinceId = PITB.CRM_API.Models.DB.DbDivision.GetById(Convert.ToInt32(divisionId)).Province_Id;
                ...
                else if (d.sTag.t2 == submitComplaintModel.tagId)
                {
                    ...
                    paramDict.Add("@Ref_Complaint_Src", PITB.CMS.Config.OtherSystemId.eTransferSIS);
                }
                else if (d.sTag.t3 == submitComplaintModel.tagId)
                {
                    paramDict.Add("@ComplaintSrc", ((int)Config.ComplaintSource.OtherSystem).ToDbObj());
                    paramDict.Add("@Ref_Complaint_Src", PITB.CMS.Config.OtherSystemId.PrivateSchoolEducation);
                }
                ...
                */
                if(isPmiuRegion)
                {
                    ...
                    divisionId = PITB.CRM_API.Models.DB.DbDistrict.GetById(Convert.ToInt32(personDistId)).Division_Id;
                    provinceId = PITB.CRM_API.Models.DB.DbDivision.GetById(Convert.ToInt32(divisionId)).Province_Id;
                    districtId = personDistId;
                }
                else
                {
                    divisionId = PITB.CRM_API.Models.DB.DbDistrict.GetById(Convert.ToInt32(submitComplaintModel.personDistrictId)).Division_Id;
                    provinceId = PITB.CRM_API.Models.DB.DbDivision.GetById(Convert.ToInt32(divisionId)).Province_Id;
                ...
                List<PITB.CMS.Models.Custom.AssignmentModel> assignmentModelList = null;
                ...
                assignmentModelList =
                    ...
                    PITB.CMS.Config.Hierarchy hierarchyId = (PITB.CMS.Config.Hierarchy)assignmentModelList[0].SrcId;
                ...
                if (listVmDynamic != null && listVmDynamic.Count > 0)
                {
                    ...
                    foreach (VmDynamicField dfField in listVmDynamic)
                    {
                        ...
                        {
                            case PITB.CMS.Config.DynamicControlType.TextBox:
                                listDynamicTextBox.Add(dfField as VmDynamicTextbox);
                                break;

                            case PITB.CMS.Config.DynamicControlType.Label:
                                listDynamicLabel.Add(dfField as VmDynamicLabel);
                                break;

                            case PITB.CMS.Config.DynamicControlType.DropDownList:
                                listDynamicDropdown.Add(dfField as VmDynamicDropDownList);
                                break;

                            case PITB.CMS.Config.DynamicControlType.DropDownListServerSideSearchable:
                    ...
                    {
                        vmComplaint.ListDynamicDropDownServerSide[i].SelectedItemId = dbSchools.Id+PITB.CMS.Config.Separator+"[" + dbSchools.school_emis_code + "] " + dbSchools.school_name;
                        /*{
                            Value = vmComplaint.ListDynamicDropDownServerSide[i].FieldName,
                            Text = dbSchools.Id+PITB.CMS.Config.Separator+"[" + dbSchools.school_emis_code + "] " + dbSchools.school_name,
                            Selected = true
                        });*/
                    }
               ...
               /* if (submitComplaintModel.PicturesList != null)
                ...
                if (d.attTag.Contains(submitComplaintModel.tagId))
                {
                    if (submitComplaintModel.listAttachmentUrl!=null)
                    {
                        ...
                        {
                            FileUploadHandler.StoreImageUrlInDb(attachment, "attachment", CMS.Config.AttachmentReferenceType.Add, ".jpg", "image/jpeg", complaintId, null, complaintId, CMS.Config.TAG_COMPLAINT_ADD, CMS.Config.AttachmentType.File);
                        }
                    }
                }
                ...
                new Thread(delegate()
                {
                    BlSchool.SendMessageToStakeholder(PITB.CMS.Models.DB.DbComplaint.GetByComplaintId(complaintId), dbSchools);
        ...
        public static GetComplaintStatuses GetMeaStatuses()
        {
            ...
            getComplaintStatuses.listDbStatuses = DbStatuses.GetByStatusIds(Config.ListMeaStatuses);
        ...
        {
            List<int?> listUserCategoryId1 = new List<int?> { (int)CMS.Config.SchoolType.Primary, (int)CMS.Config.SchoolType.Elementary };

            List<DbCrmIdsMappingToOtherSystem> listCrmUsersIdsMap = DbCrmIdsMappingToOtherSystem.Get((int)CMS.Config.CrmModule.Users, (int)CMS.Config.Hierarchy.UnionCouncil, 0, listUserCategoryId1, (int)CMS.Config.OtherSystemId.SchoolEducation, (int)CMS.Config.CrmModule.Users);

            List<DbCrmIdsMappingToOtherSystem> listCrmHierarchyIdsMap = DbCrmIdsMappingToOtherSystem.Get((int)CMS.Config.CrmModule.Hierarchy, (int)CMS.Config.OtherSystemId.SchoolEducation, (int)CMS.Config.CrmModule.Hierarchy);

            CMS.Config.Hierarchy? configHierarchy = null;
            

            if (userType == "Aeo")
            {
                configHierarchy = CMS.Config.Hierarchy.UnionCouncil;
                //listCrmHierarchyIdsMap = listCrmHierarchyIdsMap.Where(n=>n.Crm_Module_Cat1=CMS.Config.Hierarchy.)
                //listUserCategoryId1 = 
            }
            List<CMS.Models.DB.DbUsers> listDbUsers = CMS.Models.DB.DbUsers.GetUser(campaignId, configHierarchy, listUserCategoryId1, from, to);

            List<Pair<CMS.Models.DB.DbUsers,DbCrmIdsMappingToOtherSystem>> listUserMapPair = new List<Pair<DbUsers, DbCrmIdsMappingToOtherSystem>>();

            listUserMapPair = (from map in listCrmUsersIdsMap
                                  join user in listDbUsers on
                                      Convert.ToInt32(map.Crm_Id) equals user.User_Id

                                  select new Pair<CMS.Models.DB.DbUsers, DbCrmIdsMappingToOtherSystem>
                                  (
                                    user,
                                    map
                                  )).ToList();
            ...
            */
            ...
            CMS.Models.DB.DbUsers dbUser = null;
        ...
        public static PITB.CRM_API.Models.API.SchoolEducation.SISApiModel.Response.SEGetComplaintAgainstEmisCodes  GetComplaintsAgainstEmisCode(SISApiModel.Request.SEGetComplaintAgainstEmisCodes reqEmisCode)
        {
            PITB.CRM_API.Models.API.SchoolEducation.SISApiModel.Response.SEGetComplaintAgainstEmisCodes response = new SISApiModel.Response.SEGetComplaintAgainstEmisCodes();
            List<PITB.CMS.Models.DB.DbComplaint> listDbComplaint = PITB.CMS.Models.DB.DbComplaint.GetBy((int) Config.Campaign.SchoolEducationEnhanced,
                    PITB.CMS.Config.ComplaintType.Complaint, reqEmisCode.ListSchoolModel.Select(n => n.EmisCode).ToList());

            response.ListComplaint =
                listDbComplaint.Select(
                    n => new PITB.CRM_API.Models.API.SchoolEducation.SISApiModel.Response.ComplaintModel
                    {
                        Id = n.Id,
            ...
            List<PITB.CMS.Models.DB.DbComplaint> tempListDbComplaint = null;
            SISApiModel.Response.SchoolStatusWiseCount statusWiseCount = null;
            //List<PITB.CMS.Models.DB.DbComplaint> tempListDbComplaint2 = null;
            ...
            foreach (var groupVal in groupBy)
            {
                ...
                //{
                    statusWiseCount.PendingFresh = tempListDbComplaint.Where(
                        n => n.Complaint_Computed_Status_Id == (int)PITB.CMS.Config.ComplaintStatus.PendingFresh).Count();

                    statusWiseCount.Overdue = tempListDbComplaint.Where(
                        n => n.Complaint_Computed_Status_Id == (int)PITB.CMS.Config.ComplaintStatus.UnsatisfactoryClosed).Count();

                    statusWiseCount.Reopened = tempListDbComplaint.Where(
                        n => n.Complaint_Computed_Status_Id == (int)PITB.CMS.Config.ComplaintStatus.PendingReopened).Count();
                //}
