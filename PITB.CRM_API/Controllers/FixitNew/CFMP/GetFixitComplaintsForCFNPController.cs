...
{
    public class GetFixitComplaintsForCFNPController : ApiController
    {
        // GET api/<controller>
        ...
        // POST api/<controller>
        public ResponseCNFPComplaintsModel Post([FromBody]JToken jsonBody/*, [FromUri] int appId = 0*/, [FromUri] int languageId = 1/*, [FromUri] int platformId = 0*/)
        {
            ...
            try
            {
                RequestCNFPComplaintsModel reqCnfpModel =
                    (RequestCNFPComplaintsModel)JsonConvert.DeserializeObject(actualJson, typeof(RequestCNFPComplaintsModel));
                ...
                return BlFixitNew.GetCnfpComplaints(reqCnfpModel);
            }
            catch (Exception ex)
            {
