...
{
    public class SyncComplainantController : ApiController
    {
        // GET api/<controller>
        ...
        public SyncModel Post([FromBody]JToken jsonBody, [FromUri] int appId = 0, [FromUri] int languageId = 0, [FromUri] int dbVersionId = 0, [FromUri] int appVersionId = 0, [FromUri] int mobileVersionId = 0, [FromUri]  int platformId = 0)
        {
            ...
            try
            {
                SubmitSyncComplainant submitSyncComplaint =
                    (SubmitSyncComplainant)JsonConvert.DeserializeObject(actualJson, typeof(SubmitSyncComplainant));
                apiRequestId = Convert.ToInt32(BlApiHandler.StoreApiRequestInDb(actualJson, ipAddress, true, null));
                return BlApiHandler.SyncComplainant(submitSyncComplaint, appId, (Config.Language)languageId, (Config.PlatformID)platformId, dbVersionId, appVersionId);
            }
            catch (Exception ex)
            {
